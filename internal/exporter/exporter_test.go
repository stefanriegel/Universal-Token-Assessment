...
func TestBuild_SummarySubtotalAggregateThenDivide(t *testing.T) {
	...
	// Summary reports the selected current-state NIOS licensing subtotal with
	// the report growth buffer, not the full-migration Universal DDI rate used
	// by the raw provider rows.
	oracle := int(math.Ceil(float64(calculator.CeilDiv(25, calculator.NIOSTokensPerActiveIP)) * 1.20))
...
func TestBuild_TokenCalcLabelsRawRowsAsNonAdditive(t *testing.T) {
	in := testInput(threeFindings(), nil)
	f := openResult(t, in)

	g1, _ := f.GetCellValue("Token Calculation", "G1")
	h1, _ := f.GetCellValue("Token Calculation", "H1")
	h2, _ := f.GetCellValue("Token Calculation", "H2")
	i2, _ := f.GetCellValue("Token Calculation", "I2")
	if g1 != "Per-Row Tokens (Traceability)" {
		t.Errorf("G1 = %q, want traceability label", g1)
	}
	if h1 != "Authoritative Pooled Total" || h2 != strconv.Itoa(in.Totals.GrandTotal) {
		t.Errorf("authoritative pooled total = header %q value %q, want header and %d", h1, h2, in.Totals.GrandTotal)
	}
	if !strings.Contains(i2, "non-additive") {
		t.Errorf("I2 reconciliation note = %q, want non-additive explanation", i2)
	}
}
...
func TestBuild_NiosMigrationScenarios(t *testing.T) {
	...
	// Row 2: Current -- pooled NIOS total 48, then one 20% growth ceiling = 58.
	a2, _ := f.GetCellValue("NIOS Migration Scenarios", "A2")
	if a2 != "Current" {
		t.Errorf("A2 = %q, want 'Current'", a2)
	}
	b2, _ := f.GetCellValue("NIOS Migration Scenarios", "B2")
	if b2 != "58" {
		t.Errorf("B2 (current NIOS tokens) = %q, want '58'", b2)
	}

	// Row 3: Full -- pooled UDDI total 113, then one 20% growth ceiling = 136.
	a3, _ := f.GetCellValue("NIOS Migration Scenarios", "A3")
	if a3 != "Full" {
		t.Errorf("A3 = %q, want 'Full'", a3)
	}
	b3, _ := f.GetCellValue("NIOS Migration Scenarios", "B3")
	if b3 != "136" {
		t.Errorf("B3 (full UDDI tokens) = %q, want '136'", b3)
	}
...
// TestBuild_NiosServerTokens verifies the selected migration plan's sizeable members.
func TestBuild_NiosServerTokens(t *testing.T) {
	in := niosTestInput()
	in.NiosMigrationMap = map[string]string{
		"member1.grid.local": "nios-x",
		"member2.grid.local": "nios-x",
	}
	f := openResult(t, in)
	...
	// Row 2 should be selected member1 (infra-only GM and retained members excluded)
...
// TestBuild_ResourceSavingsExplicitZeroObjectOverride verifies the
// presence-aware override map reaches Resource Savings. The raw count crosses
// the 2XS object boundary, while an explicit zero must keep the member at 2XS.
func TestBuild_ResourceSavingsExplicitZeroObjectOverride(t *testing.T) {
	in := buildInputWithNiosMetrics(t)
	in.NiosServerMetrics[1].QPS = 1
	in.NiosServerMetrics[1].LPS = 1
	in.NiosServerMetrics[1].ObjectCount = 7_501
	in.NiosServerObjectOverrides = map[string]int{"phys": 0}

	f := buildAndOpen(t, in, nil)
	memberRow := findRowContaining(t, f, "Resource Savings", "A", "phys.grid.local")

	tierCol, _ := excelize.ColumnNumberToName(11)
	gotTier, _ := f.GetCellValue("Resource Savings", tierCol+strconv.Itoa(memberRow))
	if gotTier != "2XS" {
		t.Errorf("phys member New Tier = %q, want %q for explicit zero object override", gotTier, "2XS")
	}
}
...
func TestBuild_SummaryUsesSuppliedTotalsNotRecomputed(t *testing.T) {
	...
	if got, _ := f.GetCellValue("AWS", "H2"); got != "777" {
		t.Errorf("AWS!H2 (authoritative provider total) = %q, want %q", got, "777")
	}
}
