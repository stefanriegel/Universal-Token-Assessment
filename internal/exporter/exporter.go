...
// Build writes a structured .xlsx workbook to w from the given report input.
...
//
// in.VariantOverrides maps NIOS member IDs to user-selected variant indices used by
// the Resource Savings sheet (Phase 28). Leave nil to use each appliance spec's
// DefaultVariantIndex for every member.
func Build(w io.Writer, in ReportInput) error {
	f := excelize.NewFile()
	defer f.Close()
	...
	if err := buildSummarySheet(f, styles, in); err != nil {
		return err
	}
	...
	if err := buildTokenCalcSheet(f, styles, in); err != nil {
		return err
	}
	...
	// Present-but-empty is meaningful: a Grid with no members still renders the
	// NIOS sheets (empty fleet), so these conditions test for presence, not length.
	if in.NiosServerMetrics != nil {
		...
		if err := buildResourceSavingsSheet(
			f,
			styles,
			in.NiosServerMetrics,
			in.VariantOverrides,
			in.NiosServerObjectOverrides,
		); err != nil {
			return err
		}
	}
	...
	for _, p := range []string{"aws", "azure", "gcp", "ad", "nios", "bluecat", "efficientip"} {
		rows := findingsByProvider(in.Findings, p)
		if len(rows) == 0 {
			continue
		}
		...
		providerTotal := in.ProviderTotals[p]
		if p == "nios" {
			// The NIOS provider detail represents the Full migration scenario,
			// so price its rows at native Universal DDI rates and apply growth
			// once. ProviderTotals intentionally carries the selected Current
			// scenario instead and is therefore not authoritative here.
			providerTotal = applyManagementGrowth(
				calcManagementScenarioBase(rows, nil, true),
				in.GrowthBufferPct,
			)
		}
		if err := buildProviderSheet(f, styles, sheetName, rows, providerTotal); err != nil {
			return err
		}
	}
	...
	if len(in.Errors) > 0 {
		...
		if err := buildErrorsSheet(f, styles, in); err != nil {
			return err
		}
	}
	...
	if in.NiosServerMetrics != nil {
		...
		if err := buildNiosMigrationScenariosSheet(f, styles, in); err != nil {
			return err
		}
	}
	...
	if in.NiosServerMetrics != nil {
		...
		if err := buildNiosServerTokensSheet(f, styles, in); err != nil {
			return err
		}
	}
	...
	if in.NiosServerMetrics != nil {
		...
		if err := buildNiosMemberDetailsSheet(f, styles, in); err != nil {
			return err
		}
	}

	// Sheet — NIOS Microsoft Servers (conditional: Grid-managed Windows DNS/DHCP).
	if in.NiosMicrosoftServers != nil {
		if _, err := f.NewSheet("NIOS Microsoft Servers"); err != nil {
			return fmt.Errorf("exporter: new sheet NIOS Microsoft Servers: %w", err)
		}
		if err := freezeFirstRow(f, "NIOS Microsoft Servers"); err != nil {
			return fmt.Errorf("exporter: freeze panes NIOS Microsoft Servers: %w", err)
		}
		if err := buildNiosMicrosoftServersSheet(f, styles, in); err != nil {
			return err
		}
	}

	// Sheet — Microsoft Allocation (conditional: present when the NIOS scan derived an
	// allocation snapshot). D-10: its own sheet, never folded into Token Calculation.
	if in.MicrosoftAllocation != nil {
		if _, err := f.NewSheet("Microsoft Allocation"); err != nil {
			return fmt.Errorf("exporter: new sheet Microsoft Allocation: %w", err)
		}
		if err := freezeFirstRow(f, "Microsoft Allocation"); err != nil {
			return fmt.Errorf("exporter: freeze panes Microsoft Allocation: %w", err)
		}
		if err := buildMicrosoftAllocationSheet(f, styles, in); err != nil {
			return err
		}
	}

	// Sheet — AD Migration Planner (conditional: present when AD scan produced per-DC metrics).
	if in.ADServerMetrics != nil {
	...
	if err := buildSKUSheet(f, styles, in); err != nil {
		return err
	}
...
func buildSummarySheet(f *excelize.File, styles sheetStyles, in ReportInput) error {
	...
	// Token totals.
	if err := sw.SetRow("A2", []interface{}{"Grand Total Tokens", n(tot.GrandTotal)}); err != nil {
		return err
	}
	if err := sw.SetRow("A3", []interface{}{"DDI Object Tokens", n(tot.DDITokens)}); err != nil {
		return err
	}
	if err := sw.SetRow("A4", []interface{}{"Active IP Tokens", n(tot.IPTokens)}); err != nil {
		return err
	}
	if err := sw.SetRow("A5", []interface{}{"Managed Asset Tokens", n(tot.AssetTokens)}); err != nil {
		return err
	}
	...
	if !in.GeneratedAt.IsZero() {
		scanDate = in.GeneratedAt.Format("2006-01-02 15:04:05")
	}
	if err := sw.SetRow("A6", []interface{}{"Scan Date", scanDate}); err != nil {
		return err
	}

	// No input field carries a scan duration — an imported report has none. The
	// row stays so the fixed row layout below it is preserved.
	if err := sw.SetRow("A7", []interface{}{"Scan Duration", ""}); err != nil {
		return err
	}
	...
	row := 10
	for _, p := range []string{"aws", "azure", "gcp", "ad", "nios", "bluecat", "efficientip"} {
		if len(findingsByProvider(in.Findings, p)) == 0 {
			continue
		}
		providerTotal := in.ProviderTotals[p]
		if p == "nios" {
			// NIOS is the only provider whose current licensing rate differs
			// from the Universal DDI traceability rows. The Summary must show
			// the selected current-state scenario, not the all-migrated native
			// rate total exposed on the NIOS detail sheet.
			providerTotal = applyManagementGrowth(
				calcNiosTokens(findingsByProvider(in.Findings, p))+selectedMicrosoftAllocationDelta(in),
				in.GrowthBufferPct,
			)
		}
		cell, _ := excelize.CoordinatesToCellName(1, row)
		if err := sw.SetRow(cell, []interface{}{providerDisplayName(p), n(providerTotal)}); err != nil {
			return err
		}
		row++
	}

	// D-02: per-provider subtotals aggregate-then-divide independently per provider,
	// so they may not sum exactly to the Grand Total (row 2), which is the authoritative value.
	noteCell, _ := excelize.CoordinatesToCellName(1, row)
	if err := sw.SetRow(noteCell, []interface{}{
		"Note: Grand Total (above) is authoritative. Per-provider subtotals ceil independently and may not sum to it. NIOS shows the selected current-state licensing scenario; its provider detail sheet shows the full-migration Universal DDI pooled total.",
	}); err != nil {
		return err
	}
...
func buildTokenCalcSheet(f *excelize.File, styles sheetStyles, in ReportInput) error {
	...
	_ = sw.SetColWidth(7, 7, 28) // Per-row tokens
	_ = sw.SetColWidth(8, 8, 26) // Authoritative pooled total
	_ = sw.SetColWidth(9, 9, 72) // Reconciliation note

	h := func(v string) excelize.Cell { return excelize.Cell{StyleID: styles.header, Value: v} }
	n := func(v int) excelize.Cell { return excelize.Cell{StyleID: styles.number, Value: v} }

	if err := sw.SetRow("A1", []interface{}{
		h("Provider"), h("Source"), h("Region"), h("Item"), h("Count"), h("Divisor"),
		h("Per-Row Tokens (Traceability)"), h("Authoritative Pooled Total"), h("Reconciliation Note"),
	}); err != nil {
		return err
	}

	for i, row := range in.Findings {
		...
		if i == 0 {
			cells = append(cells,
				n(in.Totals.GrandTotal),
				"Authoritative total pools counts before ceiling and includes the selected scenario and growth buffer. Per-row tokens are a non-additive traceability view and may not sum to this value.",
			)
		}
	...
	if len(in.Findings) == 0 {
		if err := sw.SetRow("A2", []interface{}{
			"", "", "", "", "", "", "", n(in.Totals.GrandTotal),
			"Authoritative total pools counts before ceiling and includes the selected scenario and growth buffer. No traceability rows were supplied.",
		}); err != nil {
			return err
		}
	}
...
func buildProviderSheet(f *excelize.File, styles sheetStyles, sheetName string, rows []calculator.FindingRow, providerTotal int) error {
	...
	_ = sw.SetColWidth(7, 7, 28) // Per-row tokens
	_ = sw.SetColWidth(8, 8, 30) // Authoritative provider total
	_ = sw.SetColWidth(9, 9, 72) // Reconciliation note
	...
	if err := sw.SetRow("A1", []interface{}{
		h("Source"), h("Region"), h("Category"), h("Item"), h("Count"), h("Tokens/Unit"),
		h("Per-Row Tokens (Traceability)"), h("Authoritative Provider Pooled Total"), h("Reconciliation Note"),
	}); err != nil {
		return err
	}
	...
	for i, row := range rows {
		...
		if i == 0 {
			cells = append(cells,
				n(providerTotal),
				"Authoritative provider total pools counts before ceiling. Per-row tokens are retained for traceability and are non-additive.",
			)
		}
		if err := sw.SetRow(cell, cells); err != nil {
			return err
	...
	if len(rows) == 0 {
		if err := sw.SetRow("A2", []interface{}{
			"", "", "", "", "", "", "", n(0),
			"Authoritative provider total pools counts before ceiling. No traceability rows were supplied.",
		}); err != nil {
			return err
		}
	}
...
// buildErrorsSheet writes the Errors sheet listing all ProviderErrors from the input.
func buildErrorsSheet(f *excelize.File, styles sheetStyles, in ReportInput) error {
	...
	timestamp := "unknown"
	if !in.GeneratedAt.IsZero() {
		timestamp = in.GeneratedAt.Format("2006-01-02 15:04:05")
	}
...
// buildSKUSheet writes the Recommended SKUs sheet with an MGMT row (always) and
// SERV/REPORTING rows (each conditional on a positive supplied total). Pack
// sizes are fixed by the Infoblox SKUs; the token counts come straight from
// the report, which already applied buffering and XaaS consolidation.
func buildSKUSheet(f *excelize.File, styles sheetStyles, in ReportInput) error {
	...
	rows := []skuRow{
		{"IB-TOKENS-UDDI-MGMT-1000", "Management Token Pack (1000 tokens)", in.Totals.GrandTotal, 1000, true},
		{"IB-TOKENS-UDDI-SERV-500", "Server Token Pack (500 tokens)", in.TotalServerTokens, 500, false},
		{"IB-TOKENS-REPORTING-40", "Reporting Token Pack (40 tokens)", in.ReportingTokens, 40, false},
	}

	r := 2
	for _, s := range rows {
		if !s.always && s.tokens <= 0 {
			continue
		}
		packs := int(math.Ceil(float64(s.tokens) / float64(s.packSize)))
		cell, _ := excelize.CoordinatesToCellName(1, r)
		if err := sw.SetRow(cell, []interface{}{
			s.code,
			s.desc,
			excelize.Cell{StyleID: styles.number, Value: packs},
		}); err != nil {
			return err
		}
		r++
...
func buildADMigrationSheet(f *excelize.File, styles sheetStyles, in ReportInput) error {
	metrics := in.ADServerMetrics
...
func buildMigrationFlagsSheet(f *excelize.File, styles sheetStyles, flags *NiosMigrationFlags) error {
