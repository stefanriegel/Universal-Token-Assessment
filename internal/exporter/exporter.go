...
func Build(w io.Writer, in ReportInput) error {
	...
	if in.NiosServerMetrics != nil {
		...
		if err := buildResourceSavingsSheet(f, styles, in.NiosServerMetrics, in.VariantOverrides); err != nil {
			return err
		}
	}
...
func buildSummarySheet(f *excelize.File, styles sheetStyles, in ReportInput) error {
	...
	for _, p := range []string{"aws", "azure", "gcp", "ad", "nios", "bluecat", "efficientip"} {
		...
		if err := sw.SetRow(cell, []interface{}{providerDisplayName(p), n(in.ProviderTotals[p])}); err != nil {
			return err
		}
		row++
	}
	...
	if err := sw.SetRow(noteCell, []interface{}{
		"Note: Grand Total (above) is authoritative. Per-provider subtotals ceil independently and may not sum to it.",
	}); err != nil {
		return err
	}
