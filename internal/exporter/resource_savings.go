...
func buildResourceSavingsSheet(
	f *excelize.File,
	styles sheetStyles,
	members []NiosServerMetricFull,
	variantOverrides map[string]int,
	objectOverrides map[string]int,
) error {
	rows := computeMemberSavingsList(members, variantOverrides, objectOverrides)
...
func computeMemberSavingsList(
	...
	for i := range members {
		...
		variantIdx := -1
		if variantOverrides != nil {
			if v, ok := variantOverrides[m.MemberID]; ok {
				variantIdx = v
			}
		}

		tierName, _ := calcServerTokenTier(m.QPS, m.LPS, serverSizingObjects(m, objectOverrides))
