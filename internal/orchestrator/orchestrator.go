...
func attachNiosResults(s scanner.Scanner, sess *session.Session) {
	...
	if encoded := nrs.GetNiosMicrosoftServersJSON(); len(encoded) > 0 {
		sess.SetNiosMicrosoftServersJSON(encoded)
	}
	if encoded := nrs.GetNiosMicrosoftAllocationJSON(); len(encoded) > 0 {
		sess.SetNiosMicrosoftAllocationJSON(encoded)
	}
}
