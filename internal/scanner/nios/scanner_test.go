...
// TestNIOS_ActiveIPGridTotal verifies that the token-bearing Active IP total is
// emitted as a SINGLE grid-global-deduplicated FindingRow (not a per-member sum),
// while per-member Active IP counts remain available in NiosServerMetrics.
//
// This is the corrected design after the failover double-count fix:
// NIOS DHCP failover replicates active leases across peers, so summing per-member
// Active-IP rows double-counted them. The grid total now uses the corporate model
// (fixed + grid-global-deduped active leases + 2×networks), matching the corporate
// NIOS DB-analyzer. The per-member breakdown lives in NiosServerMetrics.ActiveIPCount
// (consumed by the per-member Excel/UI table).
func TestNIOS_ActiveIPGridTotal(t *testing.T) {
	...
	// Exactly one Active IP FindingRow (the grid total), attributed to the GM so
	// it survives the selectedMembers filter.
	...
	if len(ipRows) != 1 {
		t.Fatalf("expected exactly 1 grid-total Active IP row; got %d: %+v", len(ipRows), ipRows)
	}
	// Fixture UDDI Active IPs = 1 fixed (10.0.0.50) + 5 active leases (distinct
	// 10.0.0.1/.2/.3 + 10.0.0.20 = 4, plus corporate +1 mirror) + 2×3 network
	// reservations = 12. host_address/discovery excluded.
	if ipRows[0].Count != 12 {
		t.Errorf("grid-total Active IPs = %d, want 12 (1 fixed + 5 leases[4+mirror] + 6 reservations)", ipRows[0].Count)
	}
	if ipRows[0].Source != "gm.test.local" {
		t.Errorf("grid-total Active IP row Source = %q, want gm.test.local", ipRows[0].Source)
	}

	// Per-member Active IP counts remain available via NiosServerMetrics.
	data := any(s).(NiosResultScanner).GetNiosServerMetricsJSON()
	var metrics []niosscanner.NiosServerMetric
	if err := json.Unmarshal(data, &metrics); err != nil {
		t.Fatal(err)
