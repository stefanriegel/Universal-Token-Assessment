...
func TestNIOS_IdnsDTCMapping(t *testing.T) {
	...
	found := false
	for _, r := range rows {
		if r.Category == calculator.CategoryDDIObjects && strings.HasPrefix(r.Item, "DTC Load-Balanced Names") {
			if r.Count >= 1 {
				found = true
			}
		}
...
func TestNIOS_HostObjectDDIWeightedOne(t *testing.T) {
	...
	for _, r := range rows {
		if r.Category == calculator.CategoryDDIObjects && strings.HasPrefix(r.Item, "Host Records") {
...
func TestNIOS_MultiMemberLeaseAttribution(t *testing.T) {
	...
	// GM's own resolved DDI (excludes unresolved objects, which no longer fold into
	// the GM's ObjectCount — they still flow into grid-total management-token math
	// separately, keyed by gmHostname).
	gm := metricsByHost["gm.test.local"]
	if gm.ObjectCount != 3 {
		t.Errorf("GM ObjectCount = %d, want 3 (own resolved DDI only, unresolved excluded)", gm.ObjectCount)
	}
...
func TestNIOS_HostRouteDetection(t *testing.T) {
	...
	totalNetworks := 0
	for _, r := range rows {
		if r.Category == calculator.CategoryDDIObjects && strings.HasPrefix(r.Item, "DHCP Networks") {
...
func TestReferenceBackup_GridTotals(t *testing.T) {
	...
	for _, r := range rows {
		if r.Category == calculator.CategoryDDIObjects {
			item := strings.TrimSuffix(r.Item, " (grid-managed, unattributed)")
			ddiByItem[item] += r.Count
		}
		if r.Category == calculator.CategoryActiveIPs {
			totalActiveIPs += r.Count
		}
...
// TestNIOS_UnresolvedDDIRowLabeledUnattributed verifies that unresolved-DDI
// FindingRows (grid objects that couldn't be attributed to a specific member,
// still emitted with Source=gmHostname so they rebucket to GM's own migration
// status) carry an Item label that clearly distinguishes them from GM's own
// resolved DDI objects — while Source itself stays gmHostname (unchanged).
func TestNIOS_UnresolvedDDIRowLabeledUnattributed(t *testing.T) {
	rows := runScan(t)

	const suffix = " (grid-managed, unattributed)"
	found := false
	for _, r := range rows {
		if r.Category == calculator.CategoryDDIObjects && r.Source == "gm.test.local" && strings.HasSuffix(r.Item, suffix) {
			found = true
		}
	}
	if !found {
		t.Fatalf("expected at least one unresolved-DDI FindingRow with Source=gm.test.local and Item suffixed %q; got rows: %+v", suffix, rows)
	}
}
