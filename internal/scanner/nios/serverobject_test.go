...
// has ObjectCount 3 (own resolved DDI only — unresolved DDI no longer folds in),
// 3 active leases and 1 fixed address (ActiveIPCount 4), so ServerObjectCount = 3 + 3 + 1 = 7.
func TestServerObjectCount_DDIPlusDHCP(t *testing.T) {
	...
	if gm.ObjectCount != 3 {
		t.Fatalf("precondition: GM ObjectCount = %d, want 3", gm.ObjectCount)
	}
	if gm.ServerObjectCount != 7 {
		t.Errorf("GM ServerObjectCount = %d, want 7 (3 DDI + 3 active leases + 1 fixed)", gm.ServerObjectCount)
	}
}
