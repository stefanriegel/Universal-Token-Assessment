...
type NiosServerMetric struct {
	...
	RunsDnsDhcp       bool            `json:"runsDnsDhcp"`        // true iff enable_dns/enable_dhcp set, independent of structural GM/GMC role
...
type countResult struct {
	...
	// msLedger is the Microsoft attribution ledger accumulator (ms_ledger.go).
	// Set by Scan from state.msLedger before Pass 2 runs — not by newCountResult().
...
func (result *countResult) processObject(family string, props map[string]string, vnodeID string, vnodeMap map[string]string, gmHostname string, resolver *memberResolver) {
	...
	case NiosFamilyLease:
		...
		result.msLedger.classifyDHCPObject(NiosFamilyLease, props)
	...
	case NiosFamilyFixedAddress:
		...
		result.msLedger.classifyDHCPObject(NiosFamilyFixedAddress, props)
	...
	case NiosFamilyNetwork:
		...
		result.msLedger.classifyDHCPObject(NiosFamilyNetwork, props)
	...
	case NiosFamilyHostObject:
		...
		result.msLedger.classifyDNSObject(NiosFamilyHostObject, props)
	...
	default:
		if _, isDDI := DDIFamilies[family]; isDDI {
			...
			if _, dhcpSide := msDHCPClassifiedFamilies[family]; dhcpSide {
				result.msLedger.classifyDHCPObject(family, props)
			} else {
				result.msLedger.classifyDNSObject(family, props)
			}
		}
	}
}
