...
// GetNiosMicrosoftServersJSON returns nil for WAPI scans — MS-server detection is
// backup-only for now (live /wapi msserver query is a deferred follow-up).
func (ws *WAPIScanner) GetNiosMicrosoftServersJSON() []byte { return nil }

// GetNiosMicrosoftAllocationJSON returns nil for WAPI scans — Microsoft allocation
// derivation is backup-only for now, same as GetNiosMicrosoftServersJSON above.
func (ws *WAPIScanner) GetNiosMicrosoftAllocationJSON() []byte { return nil }
