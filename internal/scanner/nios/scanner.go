...
func (s *Scanner) Scan(_ context.Context, req scanner.ScanRequest, publish func(scanner.Event)) ([]calculator.FindingRow, error) {
	...
	// Grid-total Active IPs — a SINGLE row using the corporate methodology.
	//
	// UDDI Active IPs = fixed + active-leases + 2*networks, each category counted
	// independently and grid-globally deduped (dedups DHCP failover replicas, which
	// NIOS writes onto both peers as binding_state="active"). Emitted once under the
	// Grid Master so it survives the selectedMembers filter. Per-member Active-IP counts
	// remain available in the NiosServerMetrics table (buildMetrics) for display,
	// matching the DB-analyzer's per-member sheet (un-deduped per-member figures).
	if n := result.gridUDDIActiveIPs(); n > 0 {
		rows = append(rows, calculator.FindingRow{
			Provider:         "nios",
			Source:           gmHostname,
