...
type Scanner struct {
	...
	microsoftLedger     *MicrosoftAllocationLedger
	microsoftDiag       MSAllocationDiagnostic
	microsoftAllocation *MSAllocationScenarioSet
}
...
// niosMicrosoftAllocationJSON is the wire wrapper marshalled by
// GetNiosMicrosoftAllocationJSON. It embeds MSAllocationScenarioSet with no
// json tag so its fields inline, and adds the raw D-06 evidence counts as a
// sibling "evidence" field, without altering DeriveMSAllocationScenarios's
// own return type.
type niosMicrosoftAllocationJSON struct {
	MSAllocationScenarioSet
	Evidence MSEvidenceCounts `json:"evidence"`
}

// GetNiosMicrosoftAllocationJSON returns JSON-encoded niosMicrosoftAllocationJSON
// after Scan() completes. Unlike the other getters above, the only nil guard is
// on s.microsoftAllocation itself: a snapshot with zero scenarios (the D-07
// absent or D-16 unavailable diagnostic) is still a valid result the frontend
// must receive, so there is no additional zero-length-scenarios guard here.
func (s *Scanner) GetNiosMicrosoftAllocationJSON() []byte {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.microsoftAllocation == nil {
		return nil
	}
	var evidence MSEvidenceCounts
	if s.microsoftLedger != nil {
		evidence = s.microsoftLedger.Evidence
	}
	wrapped := niosMicrosoftAllocationJSON{
		MSAllocationScenarioSet: *s.microsoftAllocation,
		Evidence:                evidence,
	}
...
var pass1Types = map[string]struct{}{
	...
	".com.infoblox.dns.zone_ms_secondary_server":   {}, // MS-managed secondary DNS zones (D-17: never claims)
	".com.infoblox.dns.ms_dhcp_member":             {}, // DHCP network ownership relationship (A2)
}
...
func (s *Scanner) Scan(_ context.Context, req scanner.ScanRequest, publish func(scanner.Event)) ([]calculator.FindingRow, error) {
	...
	// Precompute Microsoft DNS zone attribution once Pass 1 has finished
	// populating zoneMSServer/zoneAnomaly — a zone_ms_primary_server row can
	// appear before or after the ms_server rows it references, so resolution
	// cannot happen incrementally during Pass 1 itself.
	state.msLedger.buildMSDNSResolver()
	// Same order-independence reasoning applies to DHCP network ownership
	// (ms_dhcp_member vs. ms_server_dhcp_properties).
	state.msLedger.buildMSDHCPResolver()
	// Containment resolver (D-06) must run after buildMSDHCPResolver: it
	// reuses dhcpNetworkResolution's attributable verdicts as its boundary
	// set, rather than re-deriving them.
	state.msLedger.buildMSContainmentResolver()
	...
	result.msLedger = state.msLedger
	objectCount := 0
	...
	// ---- Finalize Microsoft attribution ledger baselines against the same
	// grid-wide totals the NIOS scan itself reports, then join server counts. ----
	state.msLedger.ddiObjects.Baseline = result.gridDDI
	state.msLedger.activeIPs.Baseline = result.gridUDDIActiveIPs()
	state.msLedger.managedAssets.Baseline = 0

	// Task 3's Active-IP finalization: Attributable is the tier-2 union's
	// cardinality, read once now that Pass 2 has populated it. Then account
	// the one baseline non-object term Phase 5 makes no Microsoft claim on:
	// the corporate DB-analyzer's +1 active-lease off-by-one
	// (activeLeaseCount(), counter.go) is a pure aggregate mirror tied to no
	// real lease object, so no per-object accounting above can ever produce
	// it — credit it to Retained here, once, iff it is present in the
	// baseline (any active lease exists at all). The 2×networkCount()
	// network-reservation term is deliberately NOT added here:
	// classifyDHCPNetwork (Task 1) already credits it, split
	// attributable/retained, per each network's own verdict — adding it
	// again here would double it against ActiveIPs.Baseline and fail
	// checkMSConservation on any grid that has a network at all.
	state.msLedger.activeIPs.Attributable += len(state.msLedger.msActiveIPSet)
	if len(result.activeLeaseIPSet) > 0 {
		state.msLedger.activeIPs.Retained++
	}
	dnsManagedCount := 0
	dhcpManagedCount := 0
	eitherManagedCount := 0
	for oid := range state.ms.servers {
		dns := state.ms.dnsManaged[oid]
		dhcp := state.ms.dhcp[oid].managed
		if dns {
			dnsManagedCount++
		}
		if dhcp {
	...
	state.msLedger.servers.DNSManaged = dnsManagedCount
	state.msLedger.servers.DHCPManaged = dhcpManagedCount
	state.msLedger.servers.EitherManaged = eitherManagedCount
	...
	s.microsoftLedger, s.microsoftDiag = state.msLedger.build()
	allocation := DeriveMSAllocationScenarios(s.microsoftLedger, s.microsoftDiag, rows)
	s.microsoftAllocation = &allocation
	s.mu.Unlock()
