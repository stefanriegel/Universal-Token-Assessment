...
type pass1State struct {
	...
	// Microsoft-managed DNS/DHCP server collector (informational breakout).
	ms *msCollector

	// Microsoft attribution ledger (token-bearing, aggregate-only — see ms_ledger.go).
	msLedger *msLedgerState
}
...
func (p *pass1State) record(props map[string]string) {
	...
	case ".com.infoblox.one.ms_server":
		p.ms.record(props)
		p.msLedger.recordMSServer(props)
	case ".com.infoblox.dns.ms_server_dns_properties":
		p.ms.record(props)
		p.msLedger.recordMSServerDNSFlag(props)
	case ".com.infoblox.dns.ms_server_dhcp_properties":
		p.ms.record(props)
		p.msLedger.recordMSServerDHCPFlag(props)
	case ".com.infoblox.dns.ms_dhcp_member":
		p.msLedger.recordDHCPNetworkMSServer(props)
	case ".com.infoblox.dns.zone_ms_primary_server":
		p.ms.record(props)
		p.msLedger.recordZoneMSServer(props)
	case ".com.infoblox.dns.zone_ms_secondary_server":
		// Deliberately NOT p.ms.record(props): msCollector.record increments
		// ManagedZones only for the primary type (microsoft.go:81-82), and a
		// secondary relationship never claims a zone (D-17).
		p.msLedger.recordZoneMSSecondary(props)
	}
}
