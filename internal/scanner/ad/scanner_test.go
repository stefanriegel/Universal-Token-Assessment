...
// TestBuildADFindings_DedupAcrossDCs guards the core AD bug: DCs are replicas of
// one directory, so domain-replicated objects (users, AD-integrated zones) must be
// set-unioned across DCs, NOT summed. Summing N DCs inflates results ~N×.
func TestBuildADFindings_DedupAcrossDCs(t *testing.T) {
	mk := func(name string) *dcResult {
		return &dcResult{
			userKeys:     map[string]struct{}{"u1": {}, "u2": {}},
			zoneNames:    map[string]struct{}{"corp.local": {}},
			computerName: name,
		}
	}
	rows := buildADFindings([]*dcResult{mk("DC1.corp.local"), mk("DC2.corp.local")}, "corp.local")

	got := map[string]int{}
	for _, r := range rows {
		got[r.Item] = r.Count
		if r.Source != "corp.local" {
			t.Errorf("row %s source = %q, want corp.local", r.Item, r.Source)
		}
	}
	if got["dns_zone"] != 1 {
		t.Errorf("dns_zone = %d, want 1 (deduplicated across 2 DCs, not 2)", got["dns_zone"])
	}
}

// TestBuildADFindings_AssetTokenModel guards issue #68, aligned with the official
// Infoblox MSAD collection script (msad_cs `gen_active_ip`): every AD computer object
// counts as ONE Active IP (regardless of the sparse IPv4Address attribute), users are
// NOT a token driver, and there is no Managed-Asset row for AD.
func TestBuildADFindings_AssetTokenModel(t *testing.T) {
	dc := &dcResult{
		userKeys:     map[string]struct{}{"u1": {}, "u2": {}, "u3": {}},
		computerKeys: map[string]struct{}{"pc1": {}, "pc2": {}, "pc-no-ip": {}},
		computerName: "DC1.corp.local",
	}
	rows := buildADFindings([]*dcResult{dc}, "corp.local")

	got := map[string]int{}
	cat := map[string]string{}
	for _, r := range rows {
		got[r.Item] = r.Count
		cat[r.Item] = r.Category
	}
	if _, ok := got["user_account"]; ok {
		t.Errorf("user_account row present (%d) — bare AD users have no IP, must not drive tokens", got["user_account"])
	}
	for _, r := range rows {
		if r.Category == calculator.CategoryManagedAssets {
			t.Errorf("unexpected Managed-Asset row %q — MSAD model books AD computers as Active IPs, not assets", r.Item)
		}
	}
	if got["computer_count"] != 3 {
		t.Errorf("computer_count = %d, want 3 (ALL computer objects as Active IPs, not just IP'd)", got["computer_count"])
	}
	if cat["computer_count"] != calculator.CategoryActiveIPs {
		t.Errorf("computer_count category = %q, want Active IPs", cat["computer_count"])
	}
}
...
//     computer collection, per-DC ADServerMetric, aggregator computerKeys ───
...
// TestComputerKeysMerge verifies computerKeys merge (dedup) across DCs.
