...
type dcResult struct {
	...
	computerKeys    map[string]struct{} // Get-ADComputer results (all computer objects)
	computerName    string
...
// buildADFindings emits a single domain-wide row-set from the given DCs. AD DCs
// are replicas of ONE directory, so domain-replicated objects (users, computers,
// AD-integrated DNS zones/records) must be set-unioned across DCs, never summed —
// summing N DCs inflates management tokens ~N×. DHCP objects are naturally distinct
// per server, so union handles them correctly too. Zero-count rows are dropped.
func buildADFindings(dcs []*dcResult, source string) []calculator.FindingRow {
	...
	union := func(dst, src map[string]struct{}) {
		for k := range src {
			dst[k] = struct{}{}
		}
	}
	for _, dc := range dcs {
		union(zones, dc.zoneNames)
		union(records, dc.recordKeys)
		union(scopes, dc.scopeIDs)
		union(leases, dc.leaseKeys)
		union(reservations, dc.reservationKeys)
		union(computers, dc.computerKeys)
	}

	// AD token model (issue #68) — aligned with the official Infoblox MSAD collection
	// script (infoblox-sa-scripts, msad_cs). That tool's `gen_active_ip` counts EVERY
	// AD computer object (servers + DCs + workstations) as an Active IP, regardless of
	// whether the IPv4Address attribute is populated (it is often blank/stale in AD).
	// Users (`gen_active_user`) are collected as their own metric and do NOT drive
	// management tokens — bare AD users have no IP, so no user_account row and no
	// Managed-Asset computer row (computers are Active IPs, once each).
	candidates := []calculator.FindingRow{
		{Category: calculator.CategoryDDIObjects, Item: "dns_zone", Count: len(zones), TokensPerUnit: calculator.TokensPerDDIObject},
		{Category: calculator.CategoryDDIObjects, Item: "dns_record", Count: len(records), TokensPerUnit: calculator.TokensPerDDIObject},
		{Category: calculator.CategoryDDIObjects, Item: "dhcp_scope", Count: len(scopes), TokensPerUnit: calculator.TokensPerDDIObject},
		{Category: calculator.CategoryActiveIPs, Item: "dhcp_lease", Count: len(leases), TokensPerUnit: calculator.TokensPerActiveIP},
		{Category: calculator.CategoryActiveIPs, Item: "dhcp_reservation", Count: len(reservations), TokensPerUnit: calculator.TokensPerActiveIP},
		{Category: calculator.CategoryActiveIPs, Item: "computer_count", Count: len(computers), TokensPerUnit: calculator.TokensPerActiveIP},
	}
	...
	for _, row := range candidates {
		...
		row.Provider = scanner.ProviderAD
		row.Source = source
		row.ManagementTokens = ceilDiv(row.Count, row.TokensPerUnit)
		out = append(out, row)
	}
	return out
}

// deriveADDomain returns a domain label for the domain-wide row Source. It uses the
// domain suffix of the first FQDN DC name it finds (e.g. "DC1.corp.local" → "corp.local"),
// falling back to "AD Domain" when no DC name carries a dotted suffix.
func deriveADDomain(dcs []*dcResult) string {
	for _, dc := range dcs {
		for _, name := range []string{dc.computerName, dc.inputHost} {
...
func (s *Scanner) Scan(ctx context.Context, req scanner.ScanRequest, publish func(scanner.Event)) ([]calculator.FindingRow, error) {
	...
	// Build domain-wide FindingRows (set-unioned across DCs — AD DCs are replicas of
	// ONE directory, so per-DC counts must NOT be summed) plus per-DC ADServerMetrics
	// (server sizing IS legitimately per-server and is not summed into mgmt tokens).
	var adMetrics []adServerMetricInternal
	var selectedDCs []*dcResult
	...
	// One deduplicated domain-wide row-set (set-union across selected DCs), NOT per-DC sums.
	findings := buildADFindings(selectedDCs, deriveADDomain(selectedDCs))
...
// computerKeys populated. Every AD computer object is one Active IP (issue #68,
// aligned with the Infoblox MSAD collector's gen_active_ip), so we count objects and
// do not read the frequently-unpopulated IPv4Address attribute.
func collectComputers(ctx context.Context, client *winrm.Client) (*dcResult, error) {
	const script = `Get-ADComputer -Filter * -ErrorAction Stop | ` +
