...
describe('calcServerTierDistribution', () => {
  it('groups member counts per Server Tier, computed for every member (2XS/XS/S/M/L/XL)', () => {
    const members = [
      { qps: 0, lps: 0, objectCount: 0 }, // 2XS
      { qps: 0, lps: 0, objectCount: 0 }, // 2XS
      { qps: 40000, lps: 300, objectCount: 110000 }, // M
    ];
    const dist = calcServerTierDistribution(members);
    expect(dist).toEqual([
      { tier: '2XS', count: 2 },
      { tier: 'XS', count: 0 },
      { tier: 'S', count: 0 },
  ...
  it('returns all-zero tiers for an empty member list', () => {
    const dist = calcServerTierDistribution([]);
    expect(dist.every((d) => d.count === 0)).toBe(true);
    expect(dist.map((d) => d.tier)).toEqual(['2XS', 'XS', 'S', 'M', 'L', 'XL']);
  });
});

describe('calcServerTierDistribution serverObjectCount sizing', () => {
  it('sizes by serverObjectCount when present (leases push member past 2XS)', () => {
    // objectCount 2000 alone fits 2XS (maxObjects 3000); serverObjectCount 4000 needs XS (7500).
    const dist = calcServerTierDistribution([
      { qps: 100, lps: 50, objectCount: 2000, serverObjectCount: 4000 },
    ]);
    const byTier = Object.fromEntries(dist.map((d) => [d.tier, d.count]));
    expect(byTier['2XS']).toBe(0);
    expect(byTier['XS']).toBe(1);
  });

  it('falls back to objectCount when serverObjectCount is absent', () => {
    const dist = calcServerTierDistribution([{ qps: 100, lps: 50, objectCount: 2000 }]);
    const byTier = Object.fromEntries(dist.map((d) => [d.tier, d.count]));
    expect(byTier['2XS']).toBe(1);
  });

  it('falls back to objectCount when serverObjectCount is explicitly zero', () => {
    expect(serverSizingObjects({ objectCount: 4_000, serverObjectCount: 0 })).toBe(4_000);
    const dist = calcServerTierDistribution([
      { qps: 0, lps: 0, objectCount: 4_000, serverObjectCount: 0 },
    ]);
    const byTier = Object.fromEntries(dist.map((d) => [d.tier, d.count]));
    expect(byTier['2XS']).toBe(0);
    expect(byTier['XS']).toBe(1);
...
describe('NiosServerMetrics field drift guard', () => {
  // keyof typing rejects a renamed/removed key at compile time.
  const wireContractKeys: (keyof NiosServerMetrics)[] = [
    'memberId',
    'memberName',
    'role',
    'model',
    ...
    'runsDnsDhcp',
    'licenses',
    // managementTokens is intentionally excluded: it is a TS-only,
    // presenter-computed field (not part of the Go wire contract), per D-03.
  ];

  // vitest root is frontend/, so process.cwd() resolves to frontend/ during
  // `pnpm --dir frontend vitest run`; repo-root testdata/ is two levels up.
  const fixturePath = path.resolve(process.cwd(), '../testdata/nios-metric-fields.json');
  const fixture: string[] = JSON.parse(readFileSync(fixturePath, 'utf-8'));

  it('wireContractKeys matches the shared cross-language fixture', () => {
    expect([...wireContractKeys].sort()).toEqual([...fixture].sort());
  });

  it('every key on NiosServerMetrics (except managementTokens) is covered by wireContractKeys', () => {
    const sample: Required<NiosServerMetrics> = {
      memberId: 'm1',
      memberName: 'member-1',
      role: 'GM',
      qps: 0,
    ...
    const keys = Object.keys(sample).filter((k) => k !== 'managementTokens');
    expect([...keys].sort()).toEqual([...wireContractKeys].sort());
  });
});
