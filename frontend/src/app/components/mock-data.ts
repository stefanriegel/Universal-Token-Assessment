...
/**
 * Map a frontend TokenCategory back to the backend name. The Go exporter
 * switches on its own constants ('DDI Objects', 'Active IPs', 'Managed Assets'),
 * so findings sent to the export endpoint must carry those, not the UI's.
 */
export function toBackendCategory(category: TokenCategory): string {
  switch (category) {
    case 'DDI Object': return 'DDI Objects';
    case 'Active IP': return 'Active IPs';
    case 'Asset': return 'Managed Assets';
...
/**
 * Calculate NIOS licensing tokens for a set of findings using aggregate-then-divide.
 * Uses NIOS-specific ratios (50/25/13) and sums the per-category ceilings
 * (same sum-of-three approach as calcUddiTokensAggregated / calculator.go GrandTotal).
 */
export function calcNiosTokens(rows: FindingRow[]): number {
  const ddi = rows.filter(f => f.category === 'DDI Object').reduce((s, f) => s + f.count, 0);
  const ips = rows.filter(f => f.category === 'Active IP').reduce((s, f) => s + f.count, 0);
  const assets = rows.filter(f => f.category === 'Asset').reduce((s, f) => s + f.count, 0);
  return Math.ceil(ddi / NIOS_TOKEN_RATES['DDI Object'])
       + Math.ceil(ips / NIOS_TOKEN_RATES['Active IP'])
       + Math.ceil(assets / NIOS_TOKEN_RATES['Asset']);
}

/**
 * Current/Hybrid/Complete management-token comparison for the NIOS migration
 * mode-aware GM handling spec. Applies NIOS vs Native rate per FindingRow
 * source (member hostname) based on migration-map membership, aggregate-
 * then-divide per category (never per-row ceildiv), matching calcNiosTokens/
 * calcUddiTokensAggregated.
 */
export function calcNiosScenarios(
  findings: FindingRow[],
  niosMigrationMap: Map<string, import('./nios-calc').ServerFormFactor>,
  allMemberNames: string[],
): { current: number; hybrid: number; complete: number } {
  const scenarioTotal = (migrated: Set<string>): number => {
    const staying = findings.filter((f) => !migrated.has(f.source));
    const migrating = findings.filter((f) => migrated.has(f.source));
    return calcNiosTokens(staying) + calcUddiTokensAggregated(migrating);
  };

  return {
    current: scenarioTotal(new Set()),
    hybrid: scenarioTotal(new Set(niosMigrationMap.keys())),
    complete: scenarioTotal(new Set(allMemberNames)),
  };
}

/**
 * Marginal Delta for the Migration Planner: the actual change in each bucket's
 * pooled total when membership moves from `migrationMapBefore` to
 * `migrationMapAfter`. Ceiling division isn't additive (ceil(a)+ceil(b) !=
 * ceil(a+b)), so a member's own isolated pooled total is NOT a reliable
 * preview of what toggling it will do to the bucket totals — this recomputes
 * both bucket totals before/after and returns the diff.
 *
 * Works for both a single-member toggle (one entry differs between the two
 * maps) and a bulk toggle (`toggleAllMigration`) — pass the whole-batch
 * before/after maps and get one aggregate delta, not a sum of per-member deltas.
 */
export function computeMarginalDelta(
  findings: FindingRow[],
  migrationMapBefore: Map<string, unknown>,
  migrationMapAfter: Map<string, unknown>,
): { stayingDelta: number; migratingDelta: number } {
  const niosFindings = findings.filter((f) => f.provider === 'nios');
  const staying = (map: Map<string, unknown>) =>
    calcNiosTokens(niosFindings.filter((f) => !map.has(f.source)));
  const migrating = (map: Map<string, unknown>) =>
    calcUddiTokensAggregated(niosFindings.filter((f) => map.has(f.source)));
  return {
    stayingDelta: staying(migrationMapAfter) - staying(migrationMapBefore),
    migratingDelta: migrating(migrationMapAfter) - migrating(migrationMapBefore),
  };
}
