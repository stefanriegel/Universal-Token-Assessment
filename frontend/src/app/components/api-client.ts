...
// ─── Two-phase Device Authorization (AWS SSO, Azure Device Code) ─────────────
// These interactive flows produce a verification URL + code the user opens in
// their OWN browser. A single blocking request can't deliver the code in time
// (and no browser exists server-side in Docker), so the flow is split: start
// returns the code immediately, then we poll until the background auth completes.
...
interface DevicePollResponse extends ValidateResponse {
  status: 'pending' | 'complete' | 'error';
  message?: string;
}

// validateWithDeviceAuth runs the two-phase flow and resolves to a ValidateResponse.
// onMessage is called with the verification code/URL as soon as it is available.
export async function validateWithDeviceAuth(
  provider: string,
  authMethod: string,
  credentials: Record<string, string>,
  ...
  const startRes = await fetch(apiUrl(`/providers/${provider}/device/start`), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ authMethod, credentials, ...(forestIndex && forestIndex > 0 ? { forestIndex } : {}) }),
  });
  const start: DeviceStartResponse = await startRes.json().catch(() => ({}));
  if (!startRes.ok || start.error || !start.authId) {
    return { valid: false, error: start.error || 'Failed to start device authorization', subscriptions: [] };
  }
  if (start.message) onMessage(start.message);

  // Poll until the background auth completes (or the backend's 5-minute timeout).
  const deadline = Date.now() + 5 * 60 * 1000;
  while (Date.now() < deadline) {
    await new Promise((r) => setTimeout(r, 3000));
    const pollRes = await fetch(apiUrl(`/providers/${provider}/device/poll?authId=${encodeURIComponent(start.authId)}`));
    const poll: DevicePollResponse = await pollRes.json().catch(() => ({ status: 'error' } as DevicePollResponse));
    if (poll.status === 'pending') {
      if (poll.message) onMessage(poll.message);
      continue;
    }
    return { valid: !!poll.valid, error: poll.error, subscriptions: poll.subscriptions || [] };
  }
  return { valid: false, error: 'Device authorization timed out', subscriptions: [] };
}
...
export interface NiosMicrosoftServerAPI {
  fqdn: string;
  address: string;
  os: string;
  adDomain: string;
...
export interface NiosMicrosoftServersAPI {
  servers: NiosMicrosoftServerAPI[];
  managedZones: number;
}

// The six interfaces below mirror internal/scanner/nios's Microsoft
// allocation types field for field, via server/types.go and
// internal/exporter/types.go's independent Go redefinitions — all three
// copies are connected only by matching JSON names (MSPAR-01).

export interface MSAllocationDiagnosticAPI {
  available: boolean;
  code?: string;
  message?: string;
}

export interface MSCategoryTokensAPI {
  category: string;
  niosCount: number;
  niosRate: number;
  nativeCount: number;
  nativeRate: number;
  niosSubtotalNum: number;
  nativeSubtotalNum: number;
  subtotalDen: number;
  tokens: number;
}

export interface MSAllocationScenarioAPI {
  id: string;
  dnsEnabled: boolean;
  dhcpEnabled: boolean;
  /**
   * Exactly three, DDI / Active IP / Managed Asset in that order. Declared as a
   * tuple to mirror Go's `[3]MSCategoryTokens`, which makes the arity a
   * compile-time invariant on that side; an unbounded array would let the three
   * wire copies drift here and be read back as undefined/0 rather than failing.
   */
  categories: [MSCategoryTokensAPI, MSCategoryTokensAPI, MSCategoryTokensAPI];
  effectiveTokens: number;
  deltaTokens: number;
}

export interface MSEvidenceCountsAPI {
  relationshipRows: number;
  duplicateRelationshipRows: number;
  relationshipAnomalies: number;
}

export interface MicrosoftAllocationAPI {
  diagnostic: MSAllocationDiagnosticAPI;
  baselineTokens: number;
  scenarios: MSAllocationScenarioAPI[];
  evidence: MSEvidenceCountsAPI;
}
...
export interface ProviderErrorAPI {
  provider: string;
  resource: string;
  message: string;
}
...
/**
 * The complete report, as rendered. Mirrors `exporter.ReportInput` in Go field
 * for field — see internal/exporter/types.go; the JSON tags there are this
 * type's property names.
 *
 * Counts are effective (count overrides already applied) and totals are as
 * displayed (growth buffer already applied): the backend renders these numbers,
 * it does not derive them. `provider` and `category` carry BACKEND names
 * ('ad', 'DDI Objects'), not the UI's ('microsoft', 'DDI Object').
 *
 * Optional fields must be OMITTED rather than sent empty — the exporter gates
 * the NIOS and AD sheets on `!= nil`, so `[]` renders empty sheets.
 */
export interface ReportExportPayload {
  generatedAt: string;
  selectedProviders: string[];
  findings: FindingRowAPI[];
  totals: { ddiTokens: number; ipTokens: number; assetTokens: number; grandTotal: number };
  providerTotals: Record<string, number>;
  totalServerTokens: number;
  reportingTokens: number;
  niosServerMetrics?: NiosServerMetrics[];
  /** Per-member object overrides keyed by memberId; map presence preserves an explicit zero. */
  niosServerObjectOverrides?: Record<string, number>;
  adServerMetrics?: ADServerMetricAPI[];
  niosMigrationMap: Record<string, string>;
  adMigrationMap: Record<string, string>;
...
/**
 * POST /api/v1/export — renders the workbook from the posted report.
 *
 * Stateless: needs no scan ID, so an imported or merged session file exports
 * the same workbook a live scan does.
 */
export async function downloadReportExport(payload: ReportExportPayload): Promise<Blob> {
