...
import {
  ...
  validateWithDeviceAuth as apiValidateDevice,
  ...
  type NiosMicrosoftServersAPI,
  type MicrosoftAllocationAPI,
  type NiosQPSMember,
  type ProviderErrorAPI,
} from './api-client';
import {
  PROVIDERS,
  MOCK_SUBSCRIPTIONS,
...
import { exportSession, mergeFindings, type SessionSnapshot } from './session-io';
...
import { msAllocationDelta } from './results/ms-allocation-panel';
...
export function Wizard() {
  ...
  const [scanErrors, setScanErrors] = useState<ProviderErrorAPI[]>([]);
  // Why the last Download XLSX produced no file. The scan-mode button calls
  // downloadXlsxFromBackend directly and only re-enables itself on rejection,
  // so the wizard owns the message.
  ...
  const [niosMicrosoftServers, setNiosMicrosoftServers] = useState<NiosMicrosoftServersAPI | null>(null);
  const [microsoftServersOpen, setMicrosoftServersOpen] = useState(false);
  const [microsoftAllocation, setMicrosoftAllocation] = useState<MicrosoftAllocationAPI | null>(null);
  const [selectedMSScenario, setSelectedMSScenario] = useState<string>('none');
  const [msAllocationEvidenceOpen, setMsAllocationEvidenceOpen] = useState(false);
  ...
  const resourceSavingsDisplayMembers = useMemo(() => {
    const migrating = effectiveNiosMetrics.filter(
      (m) => niosMigrationMap.has(m.memberName) && resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized'
    );
    const all = effectiveNiosMetrics.filter((m) => resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized');
  ...
  const goNext = () => {
    ...
    if (nextIndex < STEPS.length) {
      ...
      // Session files loaded from disk carry their findings already, so there
      // is nothing to authenticate or scan. Advancing from the provider step
      // goes straight to the report — otherwise the user lands on Credentials
      // being asked to sign in to providers whose data they just loaded, with
      // Next disabled and no route forward.
      if (nextStep === 'credentials' && importedProviders.size > 0 && findings.length > 0) {
        setCurrentStep('results');
        return;
      }
  ...
  const validateCredential = useCallback(async (providerId: ProviderType) => {
    ...
    try {
      if (providerId === 'nios' && niosMode === 'backup' && niosUploadedFile) {
        ...
        // AWS SSO and Azure Device Code use the two-phase device-authorization
        // flow so the verification code is shown immediately (and works headless
        // / in Docker, where no browser can be launched server-side).
        const isDeviceFlow =
          (providerId === 'aws' && authMethod === 'sso') ||
          (providerId === 'azure' && authMethod === 'device-code') ||
          (providerId === 'gcp' && authMethod === 'browser-oauth');

        setDeviceCodeMessage('');
        const result = isDeviceFlow
          ? await apiValidateDevice(backendId, authMethod, creds, setDeviceCodeMessage)
          : await apiValidate(backendId, authMethod, creds);
        if (result.valid) {
          setCredentialStatus((prev) => ({ ...prev, [providerId]: 'valid' }));
  ...
  const startScan = useCallback(() => {
    ...
    setFindings([]);
    setBackendTokenAggregates(null);
    setMicrosoftAllocation(null);
    setSelectedMSScenario('none');
            ...
            if (status.status === 'complete') {
              ...
              if (results.niosServerMetrics && results.niosServerMetrics.length > 0) {
                setNiosServerMetrics(results.niosServerMetrics.map((m) => ({
                  ...
                  serverObjectCount: m.serverObjectCount,
                  ...
                  runsDnsDhcp:
                    'runsDnsDhcp' in m && typeof m.runsDnsDhcp === 'boolean'
                      ? m.runsDnsDhcp
                      : m.qps > 0 || m.lps > 0,
                  licenses: m.licenses ?? {},
                })));
              ...
              if (results.niosMicrosoftServers) {
                setNiosMicrosoftServers(results.niosMicrosoftServers);
              }
              if (results.microsoftAllocation) {
                setMicrosoftAllocation(results.microsoftAllocation);
              }
              setScanErrors(results.errors ?? []);
  ...
  const rawTotalTokens = useMemo(() => {
    if (Object.keys(countOverrides).length === 0 && backendTokenAggregates) {
      return backendTokenAggregates.ddiTokens
        + backendTokenAggregates.ipTokens
        + backendTokenAggregates.assetTokens;
    }
    return calcPooledTokensByCurrentRates(effectiveFindings);
  }, [effectiveFindings, countOverrides, backendTokenAggregates]);

  // Full-UDDI authoritative pooled total. Individual rows remain traceable,
  // but their independently rounded values are intentionally non-additive.
  const uddiPerRowTotal = useMemo(
    () => calcPooledTokensAtNativeRates(effectiveFindings),
    [effectiveFindings]
  );

  const niosCountsOverridden = hasNiosCountOverrides(countOverrides);

  // The backend's own pre-computed delta between the selected Microsoft
  // allocation scenario and the all-NIOS baseline (D-02). Injected into the
  // pre-buffer aggregates below so the single existing growth-buffer rounding
  // rule stays the only rounding rule — no new one is introduced here.
  const allocationDelta = useMemo(
    () => niosCountsOverridden
      ? { ddi: 0, ip: 0, asset: 0, total: 0 }
      : msAllocationDelta(microsoftAllocation, selectedMSScenario),
    [microsoftAllocation, selectedMSScenario, niosCountsOverridden]
  );
  ...
  const totalTokens = useMemo(() => {
    ...
    return Math.ceil((rawTotalTokens + allocationDelta.total) * (1 + growthBufferPct));
  }, [rawTotalTokens, growthBufferPct, allocationDelta]);
  ...
  const categoryTotals = useMemo(() => {
    ...
    if (!hasOverrides && backendTokenAggregates) {
      ...
      return {
        'DDI Object': backendTokenAggregates.ddiTokens + allocationDelta.ddi,
        'Active IP': backendTokenAggregates.ipTokens + allocationDelta.ip,
        'Asset': backendTokenAggregates.assetTokens + allocationDelta.asset,
      };
    }

    // Fallback path: exact fractional pooling per category (for count overrides).
    const totals: Record<string, number> = { 'DDI Object': 0, 'Active IP': 0, 'Asset': 0 };
    for (const category of Object.keys(totals)) {
      totals[category] = calcPooledTokensByCurrentRates(
        effectiveFindings.filter((finding) => finding.category === category),
      );
    }
    totals['DDI Object'] += allocationDelta.ddi;
    totals['Active IP'] += allocationDelta.ip;
    totals['Asset'] += allocationDelta.asset;
    return totals;
  }, [effectiveFindings, countOverrides, backendTokenAggregates, allocationDelta]);

  // The three category totals the report displays, growth buffer applied, in the
  // shape the export payload needs. Derived from categoryTotals so the workbook
  // and the screen cannot drift apart.
  //
  // Buffering per category and summing can exceed totalTokens by a token or two
  // (one extra ceiling per category), so the payload's grandTotal keeps coming
  // from totalTokens — the authoritative on-screen number — while these three
  // feed the Summary sheet's category rows. Same divergence the Summary sheet's
  // D-02 note already documents for the per-provider subtotals.
  const tokenAggregates = useMemo(() => {
    const buffer = (v: number) => Math.ceil(v * (1 + growthBufferPct));
    return {
      ddiTokens: buffer(categoryTotals['DDI Object'] ?? 0),
      ipTokens: buffer(categoryTotals['Active IP'] ?? 0),
      assetTokens: buffer(categoryTotals['Asset'] ?? 0),
    };
  }, [categoryTotals, growthBufferPct]);

  // Per-provider subtotals for the Summary sheet, keyed by BACKEND provider ID.
  // The shared exact fractional pool matches the Go calculator per provider.
  const providerTotals = useMemo(
    () => buildSelectedProviderTotals(effectiveFindings, allocationDelta, growthBufferPct),
    [effectiveFindings, allocationDelta, growthBufferPct],
  );

  // Full-migration server token count for the SKU widget and exports. Explicit
  // XaaS choices are pooled; every unselected server defaults to NIOS-X so the
  // full scenario never silently drops retained Hybrid members.
  const totalServerTokens = useMemo(() => {
    let niosTokens = 0;
    if (selectedProviders.includes('nios')) {
      const fullNiosMigrationMap = new Map(niosMigrationMap);
      effectiveNiosMetrics.forEach((metric) => {
        if (!fullNiosMigrationMap.has(metric.memberName)) {
          fullNiosMigrationMap.set(metric.memberName, 'nios-x');
        }
      });
      const sized = effectiveNiosMetrics.filter(
        (metric) => resolveGmStatus(metric, fullNiosMigrationMap).serverTokens === 'sized',
      );
      const niosX = sized.filter(
        (metric) => fullNiosMigrationMap.get(metric.memberName) !== 'nios-xaas',
      );
      const xaas = sized.filter(
        (metric) => fullNiosMigrationMap.get(metric.memberName) === 'nios-xaas',
      );
      niosTokens = niosX.reduce((sum, metric) => {
        const effective = applyServerOverrides(metric, serverMetricOverrides);
        return sum + calcServerTokenTier(
          effective.qps,
          effective.lps,
          effective.objects,
          'nios-x',
      ...
      const xaasInstances = consolidateXaasInstances(xaas.map((metric) => {
        const effective = applyServerOverrides(metric, serverMetricOverrides);
        return effective.qps !== metric.qps
          || effective.lps !== metric.lps
          || effective.objects !== serverSizingObjects(metric)
          ? {
              ...metric,
              qps: effective.qps,
              lps: effective.lps,
              objectCount: effective.objects,
              serverObjectCount: effective.objects,
              activeIPCount: 0,
            }
          : metric;
      }));
      niosTokens += xaasInstances.reduce((sum, instance) => sum + instance.totalTokens, 0);
    }

    let adTokens = 0;
    if (selectedProviders.includes('microsoft') && effectiveADMetrics.length > 0) {
      if (effectiveADMetrics.some((metric) => adMigrationMap.has(metric.hostname))) {
        // Use the same logic as the AD Server Token Calculator panel:
        // split selected DCs by form factor, consolidate XaaS instances.
        const niosXDcs = effectiveADMetrics.filter(m => adMigrationMap.get(m.hostname) !== 'nios-xaas');
        const xaasDcs  = effectiveADMetrics.filter(m => adMigrationMap.get(m.hostname) === 'nios-xaas');
        ...
        const xaasInstances = consolidateXaasInstances(xaasDcs.map(m => {
          ...
          return {
            ...
            qps: eff.qps, lps: eff.lps, objectCount: eff.objects, serverObjectCount: eff.objects, activeIPCount: 0,
            model: '', platform: '', runsDnsDhcp: true,
  ...
  }, [effectiveNiosMetrics, effectiveADMetrics, niosMigrationMap, adMigrationMap, selectedProviders, estimatorServerTokens, serverGrowthBufferPct, serverMetricOverrides]);
  ...
  const selectableNiosSources = useMemo(() => {
    return selectableNiosScenarioSources(effectiveFindings, effectiveNiosMetrics);
  }, [effectiveFindings, effectiveNiosMetrics]);

  const allSelectableNiosMigrated = areAllSelectableNiosSourcesMigrated(
    selectableNiosSources,
    niosMigrationMap,
  );
  const scenarioNiosMigrationMap = useMemo(() => {
    return filterNiosMigrationMap(selectableNiosSources, niosMigrationMap);
  }, [niosMigrationMap, selectableNiosSources]);
  const selectedNiosMigrationCount = selectableNiosSources.filter((source) => niosMigrationMap.has(source)).length;
  const selectableAdHostnames = useMemo(
    () => effectiveADMetrics.map((metric) => metric.hostname),
    [effectiveADMetrics],
  );
  const selectedAdMigrationCount = selectableAdHostnames.filter((hostname) => adMigrationMap.has(hostname)).length;
  ...
  const hybridScenario = useMemo(() => {
    const hasNiosSelections = selectedProviders.includes('nios') && selectedNiosMigrationCount > 0;
    const hasAdSelections = selectedProviders.includes('microsoft') && selectedAdMigrationCount > 0;
    if (!hasNiosSelections && !hasAdSelections) return null;

    // ── Management tokens (exact mixed-rate pooling, shared with exports) ─
    const combinedMgmtScenarios = calculateManagementScenarioTotals(
      effectiveFindings,
      new Set(scenarioNiosMigrationMap.keys()),
      allocationDelta,
      growthBufferPct,
      allSelectableNiosMigrated,
    );
    ...
    if (hasNiosSelections) {
      ...
      const selected = effectiveNiosMetrics.filter(m => niosMigrationMap.has(m.memberName) && resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized');
      ...
      const xaasInst = consolidateXaasInstances(xaas.map(m => {
        ...
        return {
          ...
          qps: eff.qps, lps: eff.lps, objectCount: eff.objects, serverObjectCount: eff.objects, activeIPCount: 0,
          model: m.model, platform: m.platform, runsDnsDhcp: m.runsDnsDhcp,
          managedIPCount: 0, staticHosts: 0, dynamicHosts: 0, dhcpUtilization: 0, licenses: {},
        };
      }));
    ...
    const selectionCount = selectedNiosMigrationCount + selectedAdMigrationCount;
    return { mgmt: combinedMgmtScenarios.hybrid, srv: Math.ceil(hybridSrv * (1 + serverGrowthBufferPct)), selectionCount };
  }, [effectiveFindings, effectiveNiosMetrics, effectiveADMetrics, niosMigrationMap, scenarioNiosMigrationMap, adMigrationMap, selectedProviders, growthBufferPct, serverGrowthBufferPct, serverMetricOverrides, allocationDelta, allSelectableNiosMigrated, selectedNiosMigrationCount, selectedAdMigrationCount]);
  ...
  const filteredTokenTotal = useMemo(
    () => calcPooledTokensByCurrentRates(filteredSortedFindings),
  ...
  const exportCSV = () => {
    const header = serializeCSVRow(['Provider', 'Source', 'Token Category', 'Item', 'Count', 'Tokens/Unit', 'Management Tokens']);
    const rows = effectiveFindings.map(
      (f) =>
        serializeCSVRow([
          PROVIDERS.find((p) => p.id === f.provider)?.name ?? f.provider,
          f.source,
          f.category,
          formatItemLabel(f.item),
          f.count,
          f.tokensPerUnit,
          f.managementTokens,
        ])
    );
    let summary = `\n\n${serializeCSVRow(['Total Management Tokens', '', '', '', '', '', totalTokens])}`;
    if (selectedProviders.includes('nios') && selectedNiosMigrationCount > 0) {
      const nf = effectiveFindings.filter((f) => f.provider === 'nios');
      const allNios = calcPooledTokensByCurrentRates(nf);
      const stayingNios = allSelectableNiosMigrated
        ? 0
        : calcPooledTokensByCurrentRates(
            nf.filter((f) => !scenarioNiosMigrationMap.has(f.source)),
          );
      ...
      summary += `\n${serializeCSVRow(['Scenario', 'Total Management Tokens', 'NIOS Licensing Tokens'])}`;
      summary += `\n${serializeCSVRow(['Current (NIOS Only)', scenarios.current, allNios])}`;
      summary += `\n${serializeCSVRow([`Hybrid (${selectedNiosMigrationCount} members migrated)`, scenarios.hybrid, stayingNios])}`;
      summary += `\n${serializeCSVRow(['Full Universal DDI', scenarios.full, 0])}`;
      summary += `\n\nMembers migrated:`;
      selectableNiosSources.forEach((src) => {
        const ff = scenarioNiosMigrationMap.get(src);
        if (ff) summary += `\n${serializeCSVRow(['', src, ff === 'nios-xaas' ? 'XaaS' : 'NIOS-X'])}`;
      });
    }
    if (selectedProviders.includes('nios')) {
      ...
      const metricsToExport = (selectedNiosMigrationCount > 0
      ...
      ).filter(m => resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized');
      if (metricsToExport.length > 0) {
        ...
        const xaasInst = consolidateXaasInstances(xaasMetrics.map(m => {
          ...
          return eff.qps !== m.qps || eff.lps !== m.lps || eff.objects !== serverSizingObjects(m)
            ? { ...m, qps: eff.qps, lps: eff.lps, objectCount: eff.objects, serverObjectCount: eff.objects, activeIPCount: 0 }
            : m;
        }));
        const hasAnyXaas = xaasMetrics.length > 0;
        summary += `\n\nServer Token Calculator`;
        summary += `\n${serializeCSVRow(['Grid Member', 'Role', 'Form Factor', 'QPS (Peak)', 'LPS (Peak)', 'Objects', 'Connections', 'Server Size', 'Allocated Tokens'])}`;
        // NIOS-X individual members
        niosXMetrics.forEach((m) => {
          const eff = applyServerOverrides(m, serverMetricOverrides);
          const tier = calcServerTokenTier(eff.qps, eff.lps, eff.objects, 'nios-x');
          summary += `\n${serializeCSVRow([m.memberName, m.role, 'NIOS-X', eff.qps, eff.lps, eff.objects, '—', tier.name, tier.serverTokens])}`;
        });
        // XaaS consolidated instances
        xaasInst.forEach((inst) => {
          summary += `\n${serializeCSVRow([`--- XaaS Instance ${xaasInst.length > 1 ? inst.index + 1 : ''} (replaces ${inst.connectionsUsed} NIOS members) ---`])}`;
          inst.members.forEach((m) => {
            const eff = applyServerOverrides(m, serverMetricOverrides);
            summary += `\n${serializeCSVRow([`  ${m.memberName}`, m.role, 'XaaS (1 conn)', eff.qps, eff.lps, eff.objects, '', '', '(consolidated)'])}`;
          });
          summary += `\n${serializeCSVRow([
            '  AGGREGATE', '', 'XaaS', inst.totalQps, inst.totalLps, inst.totalObjects,
            `${inst.connectionsUsed}/${inst.tier.maxConnections} conn`, inst.tier.name,
            inst.extraConnections > 0
              ? `${inst.totalTokens} (incl. ${inst.extraConnectionTokens} extra connection tokens)`
              : inst.totalTokens,
          ])}`;
        });
        ...
        summary += `\n${serializeCSVRow(['Total Allocated Server Tokens', '', '', '', '', '', '', '', totalST])}`;
        if (hasAnyXaas) {
          summary += `\n${serializeCSVRow([`Consolidation: ${xaasMetrics.length} NIOS members → ${xaasInst.length} XaaS instance${xaasInst.length > 1 ? 's' : ''} (${xaasMetrics.length}:${xaasInst.length} ratio)`])}`;
        }
      }
    }
    ...
    if (selectedProviders.includes('microsoft') && effectiveADMetrics.length > 0) {
      ...
      const metricsToExport = selectedAdMigrationCount > 0
      ...
      if (metricsToExport.length > 0) {
        ...
        summary += `\n${serializeCSVRow(['Hostname', 'Role', 'Form Factor', 'QPS (Peak)', 'LPS (Peak)', 'Objects', 'Connections', 'Server Size', 'Allocated Tokens'])}`;
        niosXDcs.forEach(m => {
          const eff = applyADServerOverrides(m, serverMetricOverrides);
          const tier = calcServerTokenTier(eff.qps, eff.lps, eff.objects, 'nios-x');
          summary += `\n${serializeCSVRow([m.hostname, 'DC', 'NIOS-X', eff.qps, eff.lps, eff.objects, '—', tier.name, tier.serverTokens])}`;
        });
        xaasInst.forEach(inst => {
          summary += `\n${serializeCSVRow([`--- XaaS Instance ${xaasInst.length > 1 ? inst.index + 1 : ''} (replaces ${inst.connectionsUsed} DCs) ---`])}`;
          inst.members.forEach(mem => {
            summary += `\n${serializeCSVRow([`  ${mem.memberName}`, 'DC', 'XaaS (1 conn)', mem.qps, mem.lps, mem.objectCount, '', '', '(consolidated)'])}`;
          });
          summary += `\n${serializeCSVRow([
            '  AGGREGATE', '', 'XaaS', inst.totalQps, inst.totalLps, inst.totalObjects,
            `${inst.connectionsUsed}/${inst.tier.maxConnections} conn`, inst.tier.name,
        ...
        summary += `\n${serializeCSVRow(['Total AD Allocated Server Tokens', '', '', '', '', '', '', '', adTotalST])}`;
        if (hasAnyXaas) {
          summary += `\n${serializeCSVRow([`Consolidation: ${xaasDcs.length} DCs → ${xaasInst.length} XaaS instance${xaasInst.length > 1 ? 's' : ''} (${xaasDcs.length}:${xaasInst.length} ratio)`])}`;
        }
      }
    }
    summary += `\n\nRecommended SKUs`;
    summary += `\n${serializeCSVRow(['SKU Code', 'Description', 'Pack Count'])}`;
    summary += `\n${serializeCSVRow(['Mgmt Growth Buffer', `${Math.round(growthBufferPct * 100)}%`])}`;
    if (hasServerMetrics) summary += `\n${serializeCSVRow(['Server Growth Buffer', `${Math.round(serverGrowthBufferPct * 100)}%`])}`;
    summary += `\n${serializeCSVRow(['IB-TOKENS-UDDI-MGMT-1000', 'Management Token Pack (1000 tokens)', Math.ceil(totalTokens / 1000)])}`;
    if (hasServerMetrics) {
      summary += `\n${serializeCSVRow(['IB-TOKENS-UDDI-SERV-500', 'Server Token Pack (500 tokens)', Math.ceil(totalServerTokens / 500)])}`;
    }
    if (reportingTokens > 0) {
      summary += `\n${serializeCSVRow(['IB-TOKENS-REPORTING-40', 'Reporting Token Pack (40 tokens)', Math.ceil(reportingTokens / 40)])}`;
    }
  ...
  /**
   * Legacy single-table HTML export, saved with an .xls extension.
   *
   * Demo mode only. `isDemo` means the backend is unreachable (use-backend.ts:127),
   * so there is no server to render the real multi-sheet workbook. Every other
   * export path goes through POST /api/v1/export — do not add callers here.
   */
  const exportExcel = () => {
    ...
    if (selectedProviders.includes('nios') && selectedNiosMigrationCount > 0) {
      ...
      html += `<tr><td>Current (NIOS Only)</td><td>${scenarios.current.toLocaleString()}</td><td>${allNios.toLocaleString()}</td></tr>`;
      html += `<tr style="background:#FFF3E0"><td>Hybrid (${selectedNiosMigrationCount} members migrated)</td><td><b>${scenarios.hybrid.toLocaleString()}</b></td><td>${stayingNios.toLocaleString()}</td></tr>`;
      html += `<tr><td>Full Universal DDI</td><td><b>${scenarios.full.toLocaleString()}</b></td><td>0</td></tr>`;
      html += '</table>';
      html += '<br/><p><b>Members migrated:</b></p><ul>';
      scenarioNiosMigrationMap.forEach((ff, src) => { html += `<li>${src} → ${ff === 'nios-xaas' ? 'NIOS-X as a Service' : 'NIOS-X'}</li>`; });
      html += '</ul>';
    }
    if (selectedProviders.includes('nios')) {
      const niosSources = new Set(effectiveFindings.filter((f) => f.provider === 'nios').map((f) => f.source));
  ...
  // Per-member workload edits, folded in so the workbook sizes members from
  // what the user typed rather than from the raw backup figures.
  //
  // QPS/LPS map directly. The object override is a sizing-only value, so keep
  // the raw objectCount/activeIPCount detail columns intact and carry the edit
  // through serverObjectCount, the canonical tier-sizing field.
  const withNiosOverrides = (m: NiosServerMetrics): NiosServerMetrics => {
    const eff = applyServerOverrides(m, serverMetricOverrides);
    const objectOverride = serverMetricOverrides[m.memberId]?.objects;
    return {
      ...m,
      qps: eff.qps,
      lps: eff.lps,
      serverObjectCount: objectOverride ?? m.serverObjectCount,
    };
  };

  // AD folds completely: the exporter prints DNS/DHCP object counts
  // individually and never sums them, so the `objects` override can only reach
  // the sheet through the tier — which the exporter renders verbatim rather
  // than deriving (exporter.go:671-675). Recomputing tier and tokens here is
  // what the AD panel on screen already does; skipped entirely when the host
  // has no override, so un-edited DCs keep the backend's own numbers.
  const withADOverrides = (m: ADServerMetricAPI): ADServerMetricAPI => {
    if (!serverMetricOverrides[m.hostname]) return m;
    const eff = applyADServerOverrides(m, serverMetricOverrides);
    const tier = calcServerTokenTier(eff.qps, eff.lps, eff.objects, 'nios-x');
    return { ...m, qps: eff.qps, lps: eff.lps, tier: tier.name, serverTokens: tier.serverTokens };
  };

  // The workbook must show what the report shows, so every count here is the
  // effective one and every total is the displayed one. The backend renders;
  // it does not recompute.
  const buildReportPayload = (): ReportExportPayload => ({
    generatedAt: new Date().toISOString(),
    selectedProviders: selectedProviders.map((p) => BACKEND_PROVIDER_ID[p]),
    findings: effectiveFindings.map((f) => ({
      ...f,
      provider: BACKEND_PROVIDER_ID[f.provider],
      category: toBackendCategory(f.category) as FindingRowAPI['category'],
    })),
    totals: { ...tokenAggregates, grandTotal: totalTokens },
    providerTotals,
    totalServerTokens,
    reportingTokens,
    // The exporter gates its NIOS / AD sheets on `!= nil`, not on length, so an
    // empty array renders empty sheets. effectiveNiosMetrics also falls back to
    // mock members when the Grid was never scanned — omit unless the provider is
    // genuinely part of this assessment and carries real data.
    niosServerMetrics:
      selectedProviders.includes('nios') && niosServerMetrics.length > 0
        ? effectiveNiosMetrics.map(withNiosOverrides)
        : undefined,
    niosServerObjectOverrides: Object.fromEntries(
      effectiveNiosMetrics.flatMap((member) => {
        const override = serverMetricOverrides[member.memberId]?.objects;
        return override === undefined ? [] : [[member.memberId, override]];
      }),
    ),
    adServerMetrics:
      selectedProviders.includes('microsoft') && adServerMetrics.length > 0
        ? effectiveADMetrics.map(withADOverrides)
        : undefined,
    niosMigrationMap: Object.fromEntries(scenarioNiosMigrationMap),
    adMigrationMap: Object.fromEntries(adMigrationMap),
    growthBufferPct,
    serverGrowthBufferPct,
    variantOverrides: Object.fromEntries(variantOverrides),
    errors: scanErrors,
    niosMicrosoftServers: niosMicrosoftServers ?? undefined,
    niosMigrationFlags: niosMigrationFlags ?? undefined,
    microsoftAllocation: !niosCountsOverridden
      ? microsoftAllocation ?? undefined
      : undefined,
    selectedMSScenario: !niosCountsOverridden
      ? selectedMSScenario
      : 'none',
  });

  // Download the real xlsx workbook by posting the whole report. No scan ID is
  // involved, so an imported or merged session exports exactly what a live scan
  // does. A failure surfaces and produces no file: handing the user a different
  // document than the one they asked for, unannounced, is the defect this
  // replaces.
  const downloadXlsxFromBackend = async () => {
    // Demo mode means the backend is unreachable, so there is no renderer.
    if (backend.isDemo) {
      exportExcel();
      return;
    }
    ...
    let blob: Blob;
    try {
      blob = await downloadReportExport(buildReportPayload());
    } catch (err) {
    ...
    const json = exportSession(
      {
        ...
        niosMigrationFlags: niosMigrationFlags ?? undefined,
        variantOverrides: Object.fromEntries(variantOverrides),
        microsoftAllocation: microsoftAllocation ?? undefined,
        selectedMSScenario,
      },
  ...
  // Everything restoreSession does except the step navigation, so callers that
  // want to stay on the current step (e.g. the multi-file drop zone, which lets
  // the user keep assembling files before proceeding) can reuse it.
  const applySnapshot = (snapshot: SessionSnapshot) => {
    ...
    setScanErrors(snapshot.errors ?? []);
    setNiosMicrosoftServers(snapshot.niosMicrosoftServers ?? null);
    setNiosMigrationFlags(snapshot.niosMigrationFlags ?? null);
    setVariantOverrides(new Map(Object.entries(snapshot.variantOverrides ?? {})));
    // Restore, not reset: read the saved selection rather than hard-coding it
    // off. That unconditional reset belongs to startScan (D-03) only — this
    // path's rule is the opposite one (D-09/MSPAR-02).
    setMicrosoftAllocation(snapshot.microsoftAllocation ?? null);
    setSelectedMSScenario(snapshot.selectedMSScenario ?? 'none');
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
          ...
          {currentStep === 'providers' && (
            <div>
              ...
              {/* Load one or more session files and merge them into one assessment */}
              <SessionDropZone
                onMerged={(snapshot) => applySnapshot(snapshot)}
                onCleared={() => restart()}
              />
            </div>
          )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="px-4 pb-4 pt-2">
                        ...
                        {status === 'validating' && currentAuthId === 'browser-oauth' && !deviceCodeMessage && (
                          <div className="mt-2 flex items-center gap-2 p-2.5 bg-blue-50 rounded-lg border border-blue-100">
                            <Loader2 className="w-3.5 h-3.5 text-blue-500 animate-spin shrink-0" />
                            <p className="text-[12px] text-blue-700">Starting Google sign-in...</p>
                          </div>
                        )}
                        {deviceCodeMessage && (currentAuthId === 'device-code' || currentAuthId === 'sso' || currentAuthId === 'browser-oauth') && (
          ...
          {currentStep === 'results' && !isEstimatorOnly && (
            <ResultsSurface
              ...
              wizardBag={{
                ...
                microsoftServersOpen,
                setMicrosoftServersOpen,
                microsoftAllocation,
                selectedMSScenario,
                setSelectedMSScenario,
                msAllocationEvidenceOpen,
                setMsAllocationEvidenceOpen,
                memberSavings,
                fleetSavings,
                variantOverrides,
                setVariantOverrides,
