...
export function Wizard() {
  ...
  const allocationDelta = useMemo(
    ...
    [microsoftAllocation, selectedMSScenario]
  );
  ...
  const totalTokens = useMemo(() => {
    ...
    raw += allocationDelta.total;
    return Math.ceil(raw * (1 + growthBufferPct));
  }, [effectiveFindings, growthBufferPct, countOverrides, backendTokenAggregates, allocationDelta]);
  ...
  // because that is what the exporter looks up. calcUddiTokensAggregated is the
  // same aggregate-then-divide the Go side applies per provider, so the two
  // agree row for row.
  const providerTotals = useMemo(() => {
    const totals: Record<string, number> = {};
    for (const p of new Set(effectiveFindings.map((f) => f.provider))) {
      totals[BACKEND_PROVIDER_ID[p]] = calcUddiTokensAggregated(
        effectiveFindings.filter((f) => f.provider === p)
      );
    }
    return totals;
  }, [effectiveFindings]);
  ...
  const totalServerTokens = useMemo(() => {
    const niosTokens = selectedProviders.includes('nios')
      ? effectiveNiosMetrics.filter(m => resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized').reduce((s, m) => {
          const eff = applyServerOverrides(m, serverMetricOverrides);
  ...
  //
  // QPS and LPS only. The `objects` override is a *sizing* figure —
  // objectCount + activeIPCount (serverSizingObjects, line 118) — and the
  // exporter both sums those two for tier sizing (nios_sheets.go:83) and prints
  // each one under its own label on the Member Details sheet
  // (nios_sheets.go:316). There is no split that keeps both columns honest and
  // no third field to carry the sizing figure, so it stays raw. The XaaS path
  // at line ~1739 collapses it into objectCount with activeIPCount zeroed, but
  // that feeds a synthetic consolidated instance that is never rendered
  // per-member, so it does not generalise here.
  const withNiosOverrides = (m: NiosServerMetrics): NiosServerMetrics => {
    const eff = applyServerOverrides(m, serverMetricOverrides);
    return { ...m, qps: eff.qps, lps: eff.lps };
  };
  ...
  const buildReportPayload = (): ReportExportPayload => ({
    ...
    adServerMetrics:
      selectedProviders.includes('microsoft') && adServerMetrics.length > 0
        ...
        : undefined,
    niosMigrationMap: Object.fromEntries(niosMigrationMap),
