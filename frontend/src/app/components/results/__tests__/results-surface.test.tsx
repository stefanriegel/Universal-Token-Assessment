...
import {
  buildHeroServerBreakdown,
  ResultsSurface,
  type ResultsSurfaceProps,
} from '../results-surface';
import { ResultsHero, type ResultsHeroProps } from '../results-hero';
...
describe('<ResultsHero/> server sizing', () => {
  const niosMetric = (
    over: Partial<NiosServerMetrics> = {},
  ): NiosServerMetrics => ({
    memberId: 'm1',
  ...
  const heroProps = (
    metrics: NiosServerMetrics[],
  ): ResultsHeroProps => ({
    totalManagementTokens: 0,
    totalServerTokens: 380,
    hasServerMetrics: true,
    hybridScenario: null,
    growthBufferPct: 0,
    ...
    effectiveNiosMetrics: metrics,
    effectiveADMetrics: [],
    heroCollapsed: false,
    setHeroCollapsed: vi.fn(),
  ...
  it('uses positive serverObjectCount and falls back to objectCount without adding active IPs', () => {
    const metrics = [
      niosMetric({ memberId: 'positive', memberName: 'positive.example.com' }),
      niosMetric({
        memberId: 'fallback',
        memberName: 'fallback.example.com',
        objectCount: 3_001,
        serverObjectCount: 0,
        activeIPCount: 9_000,
      }),
    ];
    const breakdown = buildHeroServerBreakdown(
      metrics,
      new Map(),
      [],
      new Map(),
    ...
    expect(breakdown).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'positive.example.com', tokens: 130 }),
      expect.objectContaining({ label: 'fallback.example.com', tokens: 250 }),
    ]));

    render(<ResultsHero {...heroProps(metrics)} />);

    const positiveRow = screen.getByText('positive.example.com').parentElement!;
    const fallbackRow = screen.getByText('fallback.example.com').parentElement!;
    expect(positiveRow.textContent).toMatch(/130\s+\(34%\)/);
    expect(fallbackRow.textContent).toMatch(/250\s+\(66%\)/);
  });
});
