...
describe('<ResultsMemberDetails/>', () => {
  ...
  it('sizes tiers from positive serverObjectCount and falls back to objectCount without adding active IPs', () => {
    const members = [
      member({
        memberId: 'positive',
        memberName: 'positive.example.com',
        qps: 1,
        lps: 1,
        objectCount: 100,
    ...
    const positiveCard = screen
      .getByText('positive.example.com')
      .closest('.rounded-lg') as HTMLElement;
    const fallbackCard = screen
      .getByText('fallback.example.com')
      .closest('.rounded-lg') as HTMLElement;
    expect(within(positiveCard).queryByText('2XS')).not.toBeNull();
    expect(within(positiveCard).queryByText('130 tokens')).not.toBeNull();
    expect(within(fallbackCard).queryByText('XS')).not.toBeNull();
    expect(within(fallbackCard).queryByText('250 tokens')).not.toBeNull();
  });
});
