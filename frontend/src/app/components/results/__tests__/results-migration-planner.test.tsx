...
describe('<ResultsMigrationPlanner/>', () => {
  ...
  it('renders Microsoft-managed servers when provided and open', () => {
    ...
    render(<ResultsMigrationPlanner {...baseProps({
      ...
      niosMicrosoftServers: {
        servers: [{
          fqdn: 'dc01.example.local', address: '192.0.2.11', os: 'Windows Server 2019',
  ...
  it('renders the supplied Current, Hybrid, and Full totals with Microsoft allocation excluded from Full', () => {
    const migrate = 'migrate.example';
    const stay = 'stay.example';
    const findings = [
      {
        provider: 'nios' as const,
        source: migrate,
        region: '',
        category: 'DDI Object' as const,
        item: 'DNS records',
        count: 115_600,
        tokensPerUnit: 50,
        managementTokens: 2_312,
      },
      {
        provider: 'nios' as const,
        source: stay,
        ...
        count: 256_950,
        tokensPerUnit: 50,
        managementTokens: 5_139,
      },
      {
        provider: 'nios' as const,
        source: stay,
        region: '',
        category: 'Active IP' as const,
        item: 'Active IPs',
        count: 209_300,
        tokensPerUnit: 25,
        managementTokens: 8_372,
      },
    ];
    ...
    render(
      <ResultsMigrationPlanner
        {...baseProps({
          members: [
            member({ memberId: 'migrate', memberName: migrate }),
            member({ memberId: 'stay', memberName: stay }),
          ],
          findings,
          effectiveFindings: findings,
          allocationDelta: 2_190,
          niosMigrationMap: new Map([[migrate, 'nios-x']]),
        })}
      />,
    );

    const managementSection = screen.getByRole('heading', { name: 'Management Tokens' }).parentElement!;
    expect(within(managementSection).getByText('18,013')).toBeTruthy();
    expect(within(managementSection).getByText('20,325')).toBeTruthy();
    // Full UDDI is 31,002; adding the 2,190 allocation again would incorrectly
    // render 33,192, so these assertions lock the no-double-count wiring.
    expect(within(managementSection).getByText('31,002')).toBeTruthy();
    expect(within(managementSection).queryByText('33,192')).toBeNull();
  });

  it('renders Current, Hybrid, and Full from one exact mixed-rate category pool', () => {
    const migrate = 'migrate.example';
    const stay = 'stay.example';
    const findings = [
      {
    ...
    const managementSection = screen.getByRole('heading', { name: 'Management Tokens' }).parentElement!;
    expect(within(managementSection).getByText('26')).toBeTruthy();
    expect(within(managementSection).getByText('46')).toBeTruthy();
    expect(within(managementSection).getByText('47')).toBeTruthy();
    expect(within(managementSection).queryByText('27')).toBeNull();
  });

  it('uses selected XaaS consolidation for the Full Universal DDI server card', () => {
    const members = [
      member({ memberId: 'm1', memberName: 'a.example.com' }),
      member({ memberId: 'm2', memberName: 'b.example.com' }),
    ];
    const niosMigrationMap = new Map<string, ServerFormFactor>([
    ...
    render(
      <ResultsMigrationPlanner
        {...baseProps({ members, niosMigrationMap })}
      />,
    );

    const serverSection = screen.getByRole('heading', { name: 'Server Tokens' }).parentElement!;
    // Both small members consolidate into one XaaS S instance (2,400 tokens).
    expect(within(serverSection).getAllByText('2,400')).toHaveLength(2);
    // The former hard-coded all-NIOS-X Full total was 2 * 130 = 260.
    expect(within(serverSection).queryByText('260')).toBeNull();
  });

  it('sizes DHCP-heavy members from serverObjectCount instead of adding active IPs again', () => {
    const members = [member({
      memberId: 'dhcp-heavy',
      memberName: 'dhcp-heavy.example.com',
      qps: 0,
      lps: 0,
      objectCount: 1,
      activeIPCount: 500_000,
      serverObjectCount: 3_001,
    })];

    render(<ResultsMigrationPlanner {...baseProps({ members })} />);

    const serverSection = screen.getByRole('heading', { name: 'Server Tokens' }).parentElement!;
    const fullCard = within(serverSection).getByText('Full Universal DDI').closest('.rounded-xl')!;
    expect(within(fullCard as HTMLElement).getByText('250')).toBeTruthy();
    expect(within(fullCard as HTMLElement).queryByText('2,700')).toBeNull();
  });

  it('falls back to objectCount when imported serverObjectCount is explicitly zero', () => {
    const members = [member({
      memberId: 'legacy-zero',
      memberName: 'legacy-zero.example.com',
      qps: 0,
      lps: 0,
      objectCount: 3_001,
      activeIPCount: 500_000,
    ...
    const fullCard = within(serverSection).getByText('Full Universal DDI').closest('.rounded-xl')!;
    expect(within(fullCard as HTMLElement).getByText('250')).toBeTruthy();
    expect(within(fullCard as HTMLElement).queryByText('130')).toBeNull();
  });

  it('includes an unselected service-running GM as default NIOS-X in Full while preserving explicit XaaS', () => {
    const members = [
      member({ memberId: 'xaas', memberName: 'xaas.example.com', qps: 0, lps: 0, serverObjectCount: 100 }),
      member({
        memberId: 'gm-service',
        memberName: 'gm-service.example.com',
        role: 'GM',
        runsDnsDhcp: true,
        qps: 0,
        lps: 0,
        serverObjectCount: 100,
      }),
      member({
        memberId: 'gm-portal',
        memberName: 'gm-portal.example.com',
        role: 'GM',
        runsDnsDhcp: false,
        qps: 0,
        lps: 0,
    ...
    const niosMigrationMap = new Map<string, ServerFormFactor>([
      ['xaas.example.com', 'nios-xaas'],
    ]);

    render(<ResultsMigrationPlanner {...baseProps({ members, niosMigrationMap })} />);

    const serverSection = screen.getByRole('heading', { name: 'Server Tokens' }).parentElement!;
    const hybridCard = within(serverSection).getByText('Hybrid').closest('.rounded-xl')!;
    const fullCard = within(serverSection).getByText('Full Universal DDI').closest('.rounded-xl')!;
    expect(within(hybridCard as HTMLElement).getByText('2,400')).toBeTruthy();
    expect(within(fullCard as HTMLElement).getByText('2,530')).toBeTruthy();
  });
  ...
  describe('usability: tier rollup, filtering, sorting', () => {
    ...
    it('uses serverObjectCount in the Server Tier distribution rollup', () => {
      const serverSizedMembers = [
        member({
          memberId: 'server-sized',
          memberName: 'server-sized.example.com',
          qps: 0,
          lps: 0,
          objectCount: 1,
          serverObjectCount: 3_001,
        ...
        member({
          memberId: 'legacy-sized',
          memberName: 'legacy-sized.example.com',
          qps: 0,
          lps: 0,
          objectCount: 1,
        }),
      ...
      render(<ResultsMigrationPlanner {...baseProps({ members: serverSizedMembers })} />);

      const rollup = screen.getByTestId('server-tier-rollup');
      expect(within(rollup).getByText(/^2XS:/).textContent).toBe('2XS: 1');
      expect(within(rollup).getByText(/^XS:/).textContent).toBe('XS: 1');
    });

    it('filters Server Tiers using serverObjectCount', async () => {
      const user = userEvent.setup();
      const serverSizedMembers = [
        member({
          memberId: 'server-sized',
      ...
      await user.selectOptions(screen.getByRole('combobox', { name: /filter by server tier/i }), 'XS');

      const planner = screen.getByTestId('migration-member-selector');
      expect(within(planner).getAllByText('server-sized.example.com').length).toBeGreaterThan(0);
      expect(within(planner).queryByText('legacy-sized.example.com')).toBeNull();
    });
