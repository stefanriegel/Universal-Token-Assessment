...
export function ResultsMigrationPlanner(props: ResultsMigrationPlannerProps): ReactElement | null {
  ...
  // source list and the `niosFindings` fed into calcNiosScenarios below always
  // come from the same underlying data — no cross-prop coupling/assumption
  // that `findings` is a superset of `effectiveFindings` for NIOS sources.
  ...
  const infraOnlySources = allNiosSources.filter(s => {
    ...
    return m ? resolveGmStatus(m, niosMigrationMap).serverTokens !== 'sized' : false;
  });
  ...
  const tierBySource = (source: string): string => {
    ...
    return calcServerTokenTier(m?.qps ?? 0, m?.lps ?? 0, m?.objectCount ?? 0, 'nios-x').name;
  };
  ...
  const tierDistribution = calcServerTierDistribution(
    effectiveNiosMetrics.map(m => ({ qps: m.qps ?? 0, lps: m.lps ?? 0, objectCount: m.objectCount ?? 0 })),
  );
  ...
  // SUM-everything scenario totals for the NIOS portion (current/hybrid/complete),
  // matching calcNiosScenarios's aggregate-then-divide methodology. allNiosSources
  // (not the migrateable-only niosSources) is passed as the "complete" member set so
  // infra-only sources are also folded into the UDDI-rate bucket in that scenario —
  // this keeps `complete` numerically identical to the prior allNiosUddiTokens value.
  const niosScenarios = calcNiosScenarios(niosFindings, niosMigrationMap, allNiosSources);
    ...
    <div data-testid="results-migration-planner" id="section-migration-planner" className="scroll-mt-6 bg-white rounded-xl border-2 border-[var(--infoblox-blue)]/30 mb-6 overflow-hidden">
      ...
      {(() => {
        ...
        const mgmtScenariosAll: ScenarioCard[] = mgmtScenarioValues
          ...
          const grownHybrid = applyGrowth(nonNiosTokens + niosScenarios.hybrid);
          return [
          {
            label: 'Current (NIOS Only)',
            primaryValue: applyGrowth(nonNiosTokens + niosScenarios.current),
          ...
          },
          {
            ...
            subLines: stayingTokens > 0 ? [
              { text: `${(grownHybrid - stayingTokens).toLocaleString()} on NIOS-X / Universal DDI`, color: '#0078d4' },
          ...
          {
            label: 'Full Universal DDI',
            primaryValue: applyGrowth(nonNiosTokens + niosScenarios.complete),
            desc: 'All NIOS members migrated to Universal DDI. Everything on Universal DDI licensing.',
          },
        ];
          })();
        ...
        const migrateableMetrics = effectiveNiosMetrics.filter(m => resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized');
