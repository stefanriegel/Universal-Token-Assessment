...
// step.
import { Fragment, useMemo, useRef, useState, type Dispatch, type ReactElement, type SetStateAction } from 'react';
import {
  Activity,
  ArrowRightLeft,
  Check,
...
// Transient toggle feedback (Marginal Delta) — see computeMarginalDelta in mock-data.ts.
// `key` is the toggled source name, or '__bulk__' for toggleAllMigration.
interface ToggleDelta {
  key: string;
  stayingDelta: number;
  migratingDelta: number;
}

function formatToggleDelta(delta: ToggleDelta): string {
  const parts: string[] = [];
  if (delta.stayingDelta !== 0) {
    parts.push(`NIOS ${delta.stayingDelta > 0 ? '+' : ''}${delta.stayingDelta.toLocaleString()}`);
  }
  if (delta.migratingDelta !== 0) {
    parts.push(`UDDI ${delta.migratingDelta > 0 ? '+' : ''}${delta.migratingDelta.toLocaleString()}`);
  }
  return parts.length > 0 ? parts.join(' / ') + ' tokens' : 'no change';
}
...
export interface ResultsMigrationPlannerProps {
  ...
  /** Selected Microsoft allocation delta. Applied until every selectable NIOS member is migrated. */
  ...
  niosMicrosoftServers: NiosMicrosoftServersAPI | null;
  microsoftServersOpen: boolean;
  setMicrosoftServersOpen: (v: boolean | ((prev: boolean) => boolean)) => void;
...
export function ResultsMigrationPlanner(props: ResultsMigrationPlannerProps): ReactElement | null {
  ...
  // Migration Bucket / Server Tier filters + member-list sort — combine with
  // memberSearchFilter (AND logic). See openspec migration-planner-usability.
  const [bucketFilter, setBucketFilter] = useState<'all' | 'staying' | 'migrating'>('all');
  const [tierFilter, setTierFilter] = useState<string>('all');
  const [sortKey, setSortKey] = useState<'none' | 'objectCount' | 'activeIPCount' | 'tier'>('none');

  // Transient Marginal Delta feedback shown on toggle (single or bulk).
  // Cleared automatically a few seconds after the toggle — see design.md:
  // Marginal Delta is computed on-demand at toggle time only, not persisted.
  const [toggleDelta, setToggleDelta] = useState<ToggleDelta | null>(null);
  const toggleDeltaTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const showToggleDelta = (key: string, delta: { stayingDelta: number; migratingDelta: number }) => {
    if (toggleDeltaTimer.current) clearTimeout(toggleDeltaTimer.current);
    setToggleDelta({ key, ...delta });
    toggleDeltaTimer.current = setTimeout(() => setToggleDelta(null), 4000);
  };
  ...
  // Derived from `effectiveFindings` (not the raw `findings` prop) so this
  // source list and canonical scenario pool always use the same data.
  ...
  const niosSources = selectableNiosScenarioSources(effectiveFindings, effectiveNiosMetrics);
  const selectableNiosSourceSet = new Set(niosSources);
  const infraOnlySources = allNiosSources.filter((source) => !selectableNiosSourceSet.has(source));
  const scenarioMigrationMap = filterNiosMigrationMap(niosSources, niosMigrationMap);
  const selectedNiosCount = niosSources.filter((source) => niosMigrationMap.has(source)).length;
  ...
  const toggleMigration = (source: string) => {
    const next = new Map(niosMigrationMap);
    const wasMigrating = next.has(source);
    if (wasMigrating) {
      next.delete(source);
    } else {
      next.set(source, 'nios-x');
    }
    showToggleDelta(source, computeMarginalDelta(effectiveFindings, niosMigrationMap, next));
    setNiosMigrationMap(next);
    if (!wasMigrating) onTierChange?.(source, 'nios-x');
  };
  ...
  // Filter sources by search term, Migration Bucket, and Server Tier —
  // all three axes combine with AND logic.
  const metricBySource = new Map(effectiveNiosMetrics.map(m => [m.memberName, m]));

  const tierBySource = (source: string): string => {
    const m = metricBySource.get(source);
    return calcServerTokenTier(
      m?.qps ?? 0,
      m?.lps ?? 0,
      m ? serverSizingObjects(m) : 0,
      'nios-x',
    ).name;
  };

  const hasActiveFilter = !!memberSearchFilter || bucketFilter !== 'all' || tierFilter !== 'all';

  let filteredSources = niosSources;
  if (memberSearchFilter) {
    filteredSources = filteredSources.filter(s => s.toLowerCase().includes(memberSearchFilter.toLowerCase()));
  }
  if (bucketFilter !== 'all') {
    filteredSources = filteredSources.filter(s => (bucketFilter === 'migrating') === niosMigrationMap.has(s));
  }
  if (tierFilter !== 'all') {
    filteredSources = filteredSources.filter(s => tierBySource(s) === tierFilter);
  }

  // Sort (object count / active IP count / Server Tier only — no token key,
  // see openspec migration-planner-usability task 3.2).
  if (sortKey !== 'none') {
    const tierRank = new Map(SERVER_TOKEN_TIERS.map((t, i) => [t.name, i]));
    filteredSources = [...filteredSources].sort((a, b) => {
      if (sortKey === 'tier') return (tierRank.get(tierBySource(b)) ?? 0) - (tierRank.get(tierBySource(a)) ?? 0);
      const ma = metricBySource.get(a);
      const mb = metricBySource.get(b);
      return (mb?.[sortKey] ?? 0) - (ma?.[sortKey] ?? 0);
    });
  }

  const tierDistribution = calcServerTierDistribution(
    effectiveNiosMetrics.map(m => ({
      qps: m.qps ?? 0,
      lps: m.lps ?? 0,
      objectCount: serverSizingObjects(m),
    })),
  );
  ...
  const toggleAllMigration = () => {
    const targets = hasActiveFilter ? filteredSources : niosSources;
    ...
    // One aggregate delta for the whole batch — never a sum of per-member deltas.
    showToggleDelta('__bulk__', computeMarginalDelta(effectiveFindings, niosMigrationMap, next));
    setNiosMigrationMap(next);
  };

  // Compute tokens by scenario
  const niosFindings = effectiveFindings.filter((f) => f.provider === 'nios');
  const stayingFindings = niosFindings.filter((f) => !scenarioMigrationMap.has(f.source));
  const stayingTokens = calcPooledTokensByCurrentRates(stayingFindings);

  const niosXCount = niosSources.filter((source) => niosMigrationMap.get(source) === 'nios-x').length;
  const xaasCount = niosSources.filter((source) => niosMigrationMap.get(source) === 'nios-xaas').length;
  const hybridDesc = selectedNiosCount > 0
    ? `${selectedNiosCount} of ${niosSources.length} members migrated${niosXCount > 0 && xaasCount > 0 ? ` (${niosXCount} NIOS-X, ${xaasCount} XaaS)` : niosXCount > 0 ? ' to NIOS-X' : ' to XaaS'}. Remaining stay on NIOS licensing.`
  ...
  return (
    <div data-testid="results-migration-planner" id="section-migration-planner" className="scroll-mt-6 bg-white rounded-xl border-2 border-[var(--infoblox-blue)]/30 mb-6 overflow-hidden">
      ...
      {/* Member selector */}
      <div data-testid="migration-member-selector" className="px-4 py-3 border-b border-[var(--border)]">
        {/* Search filter */}
        ...
        {/* Server Tier distribution rollup — every member, regardless of Migration Bucket */}
        <div data-testid="server-tier-rollup" className="flex items-center flex-wrap gap-x-2 gap-y-1 mb-2 text-[11px] text-[var(--muted-foreground)]">
          <span style={{ fontWeight: 600 }}>Server Tiers:</span>
          {tierDistribution.map(({ tier, count }, i) => (
            <Fragment key={tier}>
        ...
        {/* Migration Bucket + Server Tier filters, and sort controls */}
        <div className="flex items-center flex-wrap gap-1.5 mb-2">
          {(['all', 'staying', 'migrating'] as const).map((b) => (
            <button
              key={b}
              onClick={() => setBucketFilter(b)}
              className={`px-2 py-1 rounded text-[10px] border transition-colors capitalize ${
                bucketFilter === b
                  ? 'bg-[var(--infoblox-blue)] text-white border-[var(--infoblox-blue)]'
                  : 'border-[var(--border)] text-[var(--muted-foreground)] hover:bg-gray-50'
              }`}
            >
              {b}
            </button>
          ...
          <select
            ...
            aria-label="Filter by Server Tier"
            className="px-2 py-1 rounded text-[10px] border border-[var(--border)] text-[var(--muted-foreground)]"
          >
          ...
          <span className="text-[10px] text-[var(--muted-foreground)]">Sort:</span>
          {([
            ...
            <button
              key={key}
              onClick={() => setSortKey((prev) => (prev === key ? 'none' : key))}
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[11px] text-[var(--muted-foreground)]">
            {memberSearchFilter
              ...
              : `${selectedNiosCount} of ${niosSources.length} members marked for migration`}
            {selectedNiosCount > 0 && !memberSearchFilter && (() => {
              const nx = Array.from(scenarioMigrationMap.values()).filter(v => v === 'nios-x').length;
              const xs = Array.from(scenarioMigrationMap.values()).filter(v => v === 'nios-xaas').length;
              if (nx > 0 && xs > 0) return ` (${nx} NIOS-X, ${xs} XaaS)`;
              if (xs > 0) return ` (${xs} XaaS)`;
              return ` (${nx} NIOS-X)`;
            })()}
          </span>
          {toggleDelta?.key === '__bulk__' && (
            <span data-testid="migration-toggle-delta-bulk" className="text-[11px] font-semibold text-[var(--infoblox-blue)]">
              {formatToggleDelta(toggleDelta)}
            </span>
          )}
        </div>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          {resolveGmStatus(metric, niosMigrationMap).label && (
                  ...
                  <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                    <span className="text-[10px] text-[var(--muted-foreground)]">
                      {isMigrating ? 'Migrating' : 'Staying (NIOS Licensing)'}
                    </span>
                    {toggleDelta?.key === source && (
                      <span data-testid="migration-toggle-delta" className="text-[10px] font-semibold text-[var(--infoblox-blue)]">
                        {formatToggleDelta(toggleDelta)}
                      </span>
                    )}
                  </div>
          ...
          <div className="mt-3 px-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] text-slate-400">
                Replaced by Infoblox Portal — no NIOS-X licensing needed
              </span>
            </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-slate-400 italic">
                        {(m && resolveGmStatus(m, niosMigrationMap).label) || 'Replaced by Infoblox Portal'}
                      </div>
                    </div>
                  </div>
                );
      ...
      {(() => {
        const niosIsActive = (idx: number) =>
          idx === 0 ? selectedNiosCount === 0
          : idx === 1 ? selectedNiosCount > 0 && !allSelectableNiosMigrated
          : allSelectableNiosMigrated;

        // Management Token scenarios use the same combined allocation/growth
        // helper as the hero and exports.
        ...
        const mgmtScenariosAll: ScenarioCard[] = mgmtScenarioValues
          ...
          : (() => {
          // grownHybrid is the single source of truth for both the Hybrid card's
          // headline and its sublines — the UDDI-portion subline is derived as the
          // remainder (grownHybrid - stayingTokens) so the two lines always sum
          // exactly to the headline. Applying growth independently to the two
          // pieces would NOT reconcile: ceil(a)+ceil(b) != ceil(a+b) in general.
          const combinedScenarios = calculateManagementScenarioTotals(
            effectiveFindings,
            new Set(scenarioMigrationMap.keys()),
            { total: allocationDelta },
            growthBufferPct,
            allSelectableNiosMigrated,
          );
          const grownHybrid = combinedScenarios.hybrid;
          const effectiveStayingTokens = allSelectableNiosMigrated ? 0 : stayingTokens;
          return [
          {
            label: 'Current (NIOS Only)',
            primaryValue: combinedScenarios.current,
            desc: 'Cloud/MS sources need UDDI tokens; NIOS stays on traditional licensing (shown here as its NIOS-rate token equivalent).',
          },
          {
            label: 'Hybrid',
            primaryValue: grownHybrid,
            subLines: effectiveStayingTokens > 0 ? [
              { text: `${(grownHybrid - effectiveStayingTokens).toLocaleString()} on NIOS-X / Universal DDI`, color: '#0078d4' },
              { text: `${effectiveStayingTokens.toLocaleString()} on NIOS Licensing`, color: '#6b7280' },
            ] : [],
            desc: hybridDesc,
          },
          {
            label: 'Full Universal DDI',
            primaryValue: combinedScenarios.full,
            desc: 'All NIOS members migrated to Universal DDI. Everything on Universal DDI licensing.',
          },
        ];
          })();
        ...
        // Full: all members, preserving explicit XaaS targets and defaulting
        // unselected members to NIOS-X. Infra-only GM/GMC are replaced by Portal.
        const fullMigrationMap = new Map(niosMigrationMap);
        effectiveNiosMetrics.forEach((m) => {
          if (!fullMigrationMap.has(m.memberName)) fullMigrationMap.set(m.memberName, 'nios-x');
        });
        const fullMigrateableMetrics = effectiveNiosMetrics.filter(
          m => resolveGmStatus(m, fullMigrationMap).serverTokens === 'sized',
        );
        const hybridMigrateableMetrics = effectiveNiosMetrics.filter(
          m => niosMigrationMap.has(m.memberName) && resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized',
        );
        const fullSrvTokens = calcNiosServerScenario(fullMigrateableMetrics);
        const hybridSrvTokens = hybridMigrateableMetrics.length > 0
          ? calcNiosServerScenario(hybridMigrateableMetrics)
          : 0;
      ...
      {effectiveNiosMetrics.length > 0 && (() => {
        ...
        const migratingMembers = effectiveNiosMetrics.filter((m) =>
          niosMigrationMap.has(m.memberName) && resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized'
        );
        ...
        const allMembers = effectiveNiosMetrics.filter((m) => resolveGmStatus(m, niosMigrationMap).serverTokens === 'sized');
      ...
      {/* Microsoft-Managed Servers — Grid-managed Windows DNS/DHCP (informational breakout) */}
      {niosMicrosoftServers && niosMicrosoftServers.servers.length > 0 && (
        <div className="border-t border-blue-200 bg-blue-50/20">
          <button
            type="button"
            onClick={() => setMicrosoftServersOpen(!microsoftServersOpen)}
            className="w-full px-4 py-3 flex items-center gap-2 hover:bg-blue-50/50 transition-colors text-left"
          >
            {microsoftServersOpen ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
            <h3 className="text-[14px]" style={{ fontWeight: 600 }}>
              Microsoft-Managed Servers
            </h3>
            <span className="text-[11px] text-[var(--muted-foreground)]">
              ({niosMicrosoftServers.servers.length} server{niosMicrosoftServers.servers.length !== 1 ? 's' : ''}, {niosMicrosoftServers.managedZones} DNS zone{niosMicrosoftServers.managedZones !== 1 ? 's' : ''}, {niosMicrosoftServers.servers.reduce((sum, s) => sum + s.dhcpHosts, 0)} DHCP hosts)
            </span>
          </button>
            ...
            <div className="px-4 pb-4">
              <div className="overflow-x-auto max-h-[300px] overflow-y-auto rounded border border-[var(--border)]">
                <table className="w-full text-[12px]">
                  <thead className="sticky top-0 z-10">
                    <tr className="border-b border-[var(--border)] bg-gray-50">
                      <th className="text-left px-3 py-2" style={{ fontWeight: 600 }}>Server (FQDN)</th>
                      <th className="text-left px-3 py-2" style={{ fontWeight: 600 }}>IP</th>
                      <th className="text-left px-3 py-2" style={{ fontWeight: 600 }}>OS</th>
                      <th className="text-left px-3 py-2" style={{ fontWeight: 600 }}>AD Domain</th>
                      <th className="text-center px-3 py-2" style={{ fontWeight: 600 }}>DNS</th>
                      <th className="text-center px-3 py-2" style={{ fontWeight: 600 }}>DHCP</th>
                    </tr>
                  </thead>
                  <tbody>
                    {niosMicrosoftServers.servers.map((s, idx) => (
                      <tr key={idx} className="border-b border-[var(--border)] hover:bg-gray-50/50">
                        <td className="px-3 py-1.5 font-mono text-[11px]">{s.fqdn}</td>
                        <td className="px-3 py-1.5 font-mono text-[11px]">{s.address}</td>
                        <td className="px-3 py-1.5">{s.os}</td>
                        <td className="px-3 py-1.5">{s.adDomain}</td>
                        <td className="text-center px-3 py-1.5">
                          {s.dnsManaged
                            ? <span className="inline-block px-2 py-0.5 rounded-full text-[10px] bg-green-100 text-green-800" style={{ fontWeight: 600 }}>Managed</span>
