...
import { MSAllocationPanel, msAllocationDelta } from './ms-allocation-panel';
...
import { buildReconciledSourceBreakdown, hasNiosCountOverrides } from '../scenario-parity';
...
export interface HeroServerBreakdownEntry {
  label: string;
  color: string;
  tokens: number;
}

/** Build the Hero's additive server-token rows using the same XaaS pooling rules as its headline. */
export function buildHeroServerBreakdown(
  niosMetrics: NiosServerMetrics[],
  niosMigrationMap: Map<string, ServerFormFactor>,
  adMetrics: ADServerMetrics[],
  adMigrationMap: Map<string, ServerFormFactor>,
  overrides: Record<string, { qps?: number; lps?: number; objects?: number }>,
): HeroServerBreakdownEntry[] {
  const rows: HeroServerBreakdownEntry[] = [];
  const fullNiosMigrationMap = new Map(niosMigrationMap);
  niosMetrics.forEach((metric) => {
    if (!fullNiosMigrationMap.has(metric.memberName)) {
      fullNiosMigrationMap.set(metric.memberName, 'nios-x');
    }
  });
  const sizedNios = niosMetrics.filter(
    (metric) => resolveGmStatus(metric, fullNiosMigrationMap).serverTokens === 'sized',
  );
  const niosX = sizedNios.filter((metric) => fullNiosMigrationMap.get(metric.memberName) !== 'nios-xaas');
  const xaas = sizedNios.filter((metric) => fullNiosMigrationMap.get(metric.memberName) === 'nios-xaas');

  niosX.forEach((metric) => {
    const effective = applyServerOverrides(metric, overrides);
    const tokens = calcServerTokenTier(effective.qps, effective.lps, effective.objects, 'nios-x').serverTokens;
    if (tokens > 0) rows.push({ label: metric.memberName, color: '#00a5e5', tokens });
  });
  consolidateXaasInstances(xaas.map((metric) => {
    const effective = applyServerOverrides(metric, overrides);
    ...
    rows.push({
      label: `NIOS XaaS instance ${instance.index + 1} (${instance.members.length} members)`,
      color: '#7c3aed',
      tokens: instance.totalTokens,
    });
  });

  const adNiosX = adMetrics.filter((metric) => adMigrationMap.get(metric.hostname) !== 'nios-xaas');
  const adXaas = adMetrics.filter((metric) => adMigrationMap.get(metric.hostname) === 'nios-xaas');
  adNiosX.forEach((metric) => {
    const effective = applyADServerOverrides(metric, overrides);
    const tokens = calcServerTokenTier(effective.qps, effective.lps, effective.objects, 'nios-x').serverTokens;
    if (tokens > 0) rows.push({ label: metric.hostname, color: '#0078d4', tokens });
  });
  consolidateXaasInstances(adXaas.map((metric) => {
    const effective = applyADServerOverrides(metric, overrides);
    return {
      memberId: metric.hostname,
      memberName: metric.hostname,
      role: 'DC',
      qps: effective.qps,
      lps: effective.lps,
      objectCount: effective.objects,
    ...
    rows.push({
      label: `Microsoft XaaS instance ${instance.index + 1} (${instance.members.length} servers)`,
      color: '#7c3aed',
      tokens: instance.totalTokens,
    });
  });
...
export interface ResultsSurfaceWizardBag {
  ...
  microsoftAllocation: MicrosoftAllocationAPI | null;
  selectedMSScenario: string;
  setSelectedMSScenario: (v: string) => void;
  msAllocationEvidenceOpen: boolean;
  setMsAllocationEvidenceOpen: (v: boolean | ((prev: boolean) => boolean)) => void;
  ...
  /** Why the last Download XLSX produced no file; null once one succeeds. */
...
function ScanResultsSurface({ wizardBag }: { wizardBag: ResultsSurfaceWizardBag }) {
  ...
  const selectedAllocationDelta = niosCountsOverridden
    ? { ddi: 0, ip: 0, asset: 0, total: 0 }
    : msAllocationDelta(microsoftAllocation, selectedMSScenario);
  const hasFindingsFilter = findingsProviderFilter.size > 0 || findingsCategoryFilter.size > 0;
  const displayedFindings = hasFindingsFilter ? filteredSortedFindings : effectiveFindings;
  const niosRowSubtotal = displayedFindings.reduce((sum, finding) => sum + finding.managementTokens, 0);
  const uddiRowSubtotal = displayedFindings.reduce(
    (sum, finding) => sum + Math.ceil(finding.count / TOKEN_RATES[finding.category as TokenCategory]),
    0,
  );
  const pooledUddiTotal = hasFindingsFilter
    ? calcUddiTokensAggregated(displayedFindings)
    : uddiPerRowTotal;
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* PLACEHOLDER — replaced by sed-merged content in subsequent edit */}
              ...
              <div id="section-overview" className="scroll-mt-6 bg-white rounded-xl border-2 border-[var(--infoblox-orange)]/30 p-5 mb-6">
                ...
                {/* Microsoft allocation — switches, delta, held-back disclosure (D-01/D-02/D-04/D-05/D-06/D-07/D-08) */}
                <MSAllocationPanel
                  allocation={microsoftAllocation}
                  selected={niosCountsOverridden ? 'none' : selectedMSScenario}
                  onSelect={setSelectedMSScenario}
                  selectionDisabled={niosCountsOverridden}
                  selectionDescriptionId="ms-allocation-count-override-note"
                  evidenceOpen={msAllocationEvidenceOpen}
                  setEvidenceOpen={setMsAllocationEvidenceOpen}
                />
                {niosCountsOverridden && microsoftAllocation && (
                  <div id="ms-allocation-count-override-note" className="mt-2 text-base text-amber-700">
                    Microsoft allocation is temporarily excluded because manual count overrides invalidate the backup-derived delta.
                  </div>
                )}
                  ...
                  <div className={`mt-4 pt-4 border-t border-[var(--border)] grid gap-6 ${hasServerMetrics ? 'grid-cols-2' : 'grid-cols-1'}`}>
                    ...
                    <div>
                      ...
                      <div className="space-y-2.5">
                        {(() => {
                          const sources = buildReconciledSourceBreakdown(
                            effectiveFindings,
                            rawTotalTokens,
                            selectedAllocationDelta,
                            totalTokens,
                          );
                          const LIMIT = 10;
                          const sourceRows = sources.filter((entry) => entry.kind === 'source');
                          const adjustmentRows = sources.filter((entry) => entry.kind !== 'source');
                          const visible = [
                            ...(showAllHeroSources ? sourceRows : sourceRows.slice(0, LIMIT)),
                            ...adjustmentRows,
                          ];
                          const hidden = sourceRows.length - LIMIT;
                          const needsScroll = showAllHeroSources && sourceRows.length > 15;
                          return (
                            <>
                              <div className={needsScroll ? 'max-h-[400px] overflow-y-auto' : ''}>
                                {visible.map((entry) => {
                                  ...
                                  const color = provider?.color
                                    ?? (entry.kind === 'allocation' ? '#0078d4' : entry.kind === 'growth' ? '#f59e0b' : '#64748b');
                                  const pct = totalTokens > 0 ? (Math.abs(entry.tokens) / totalTokens) * 100 : 0;
                                  return (
                                      ...
                                      <div className="flex items-center justify-between mb-1">
                                        <span className="text-[12px] flex items-center gap-1.5" style={{ fontWeight: 500 }}>
                                          ...
                                          <span className="text-[11px] text-[var(--muted-foreground)]" style={{ fontWeight: 400 }}>
                                            {provider?.name ?? (entry.kind === 'pooling' ? 'non-additive row correction' : 'scenario adjustment')}
                                          </span>
                                        </span>
                                        <span className="text-[12px] tabular-nums text-[var(--muted-foreground)]">
                                          {entry.kind !== 'source' && entry.tokens > 0 ? '+' : ''}{entry.tokens.toLocaleString()} <span className="text-[11px]">({Math.round(pct)}%)</span>
                                        </span>
                                      </div>
                    ...
                    {/* Server breakdown */}
                    {hasServerMetrics && (() => {
                      const srvSources = buildHeroServerBreakdown(
                        selectedProviders.includes('nios') ? effectiveNiosMetrics : [],
                        niosMigrationMap,
                        selectedProviders.includes('microsoft') ? effectiveADMetrics : [],
                        adMigrationMap,
                        serverMetricOverrides,
                      );
                      const LIMIT = 10;
              ...
              <ResultsMigrationPlanner
                ...
                allocationDelta={selectedAllocationDelta.total}
                ...
                microsoftServersOpen={microsoftServersOpen}
                setMicrosoftServersOpen={setMicrosoftServersOpen}
              />
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-[13px]">
                    ...
                    <tbody>
                      ...
                      <tr className="bg-gray-50/60 text-[var(--muted-foreground)]">
                        <td colSpan={5} className="px-4 py-2 text-right" style={{ fontWeight: 500 }}>
                          Per-row subtotal (non-additive evidence)
                        </td>
                        <td className="px-4 py-2 text-right tabular-nums">
                          {niosRowSubtotal.toLocaleString()}
                        </td>
                        {selectedProviders.includes('nios') && (
                          <td className="px-4 py-2 text-right tabular-nums text-[var(--infoblox-orange)]">
                            {uddiRowSubtotal.toLocaleString()}
                          </td>
                        )}
                      </tr>
                      <tr className="bg-gray-50">
                        <td
                          ...
                          style={{ fontWeight: 600 }}
                        >
                          {hasFindingsFilter ? 'Authoritative Filtered Pooled Total' : 'Authoritative Pooled Total'}
                        </td>
                        <td
                          className="px-4 py-3 text-right"
                          style={{ fontWeight: 700 }}
                        >
                          {(hasFindingsFilter ? filteredTokenTotal : rawTotalTokens).toLocaleString()}
                        </td>
                        {selectedProviders.includes('nios') && (
                          <td
                            ...
                            style={{ fontWeight: 700 }}
                          >
                            {pooledUddiTotal.toLocaleString()}
                          </td>
                        )}
                      </tr>
                      <tr>
                        <td colSpan={selectedProviders.includes('nios') ? 7 : 6} className="px-4 py-2 text-right text-base text-[var(--muted-foreground)]">
                          Detail rows are independently rounded and may not add up. Footer totals use authoritative pooled calculation.
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              {/* Export buttons. A failed export reports next to the button
                  that triggered it — a banner at the top of a long report is
                  one the user may never scroll back to, which is the silent
                  failure this path exists to remove. */}
              {exportError && (
                <div
                  role="alert"
                  className="mb-3 flex items-start gap-2 p-3 bg-red-50 rounded-lg border border-red-200"
                >
                  <AlertCircle className="w-4 h-4 text-red-500 mt-0.5 shrink-0" />
                  <p className="text-[13px] text-red-700" style={{ fontWeight: 500 }}>
