...
describe('VNIOS_SPECS table', () => {
  ...
  it('contains all 5 X6/VMware models', () => {
    // Scoped to the IB-V series; ND-V1606 is also X6/VMware but not part of the IB-V lineup.
    expect(VNIOS_SPECS.filter(s => s.generation === 'X6' && s.platform === 'VMware' && s.model.startsWith('IB-V'))).toHaveLength(5);
  });

  it('contains all 5 X6/Azure models', () => {
    expect(VNIOS_SPECS.filter(s => s.generation === 'X6' && s.platform === 'Azure' && s.model.startsWith('IB-V'))).toHaveLength(5);
  });

  it('maps ND-V1606 on VMware (16/64) and Azure (20/160)', () => {
    const vmware = lookupApplianceSpec('ND-V1606', 'VMware');
    expect(vmware?.variants[0]).toMatchObject({ vCPU: 16, ramGB: 64 });
    const azure = lookupApplianceSpec('ND-V1606', 'Azure');
    expect(azure?.variants[0]).toMatchObject({ vCPU: 20, ramGB: 160 });
  });
