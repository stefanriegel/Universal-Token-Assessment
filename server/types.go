...
type ScanResultsResponse struct {
	...
	// NiosMicrosoftServers is populated when the NIOS backup scan found Grid-managed
	// Microsoft (Windows) DNS/DHCP servers.
	NiosMicrosoftServers *NiosMicrosoftServers `json:"niosMicrosoftServers,omitempty"`
	// MicrosoftAllocation is populated when the NIOS backup scan derived a
	// Microsoft allocation snapshot. nil when NIOS was not scanned.
...
// NiosMicrosoftServers is the grid-wide Microsoft-managed summary from a NIOS backup scan.
...
// available for this scan, and if not, why. This is an independent
// redefinition of internal/scanner/nios.MSAllocationDiagnostic — the two are
// connected only by matching JSON tags, never a shared import.
...
// field — the three copies (this one, internal/exporter's, and the
// TypeScript client's) are connected only by matching JSON names.
