...
func (h *ScanHandler) HandleScanResults(w http.ResponseWriter, r *http.Request) {
	...
	// Decode NiosMicrosoftServersJSON if the NIOS backup scan found MS-managed servers.
	var niosMicrosoftServers *NiosMicrosoftServers
	if len(sess.NiosMicrosoftServersJSON) > 0 {
		niosMicrosoftServers = &NiosMicrosoftServers{}
		if err := json.Unmarshal(sess.NiosMicrosoftServersJSON, niosMicrosoftServers); err != nil {
			fmt.Fprintf(os.Stderr, "server: failed to decode NiosMicrosoftServersJSON: %v\n", err)
			niosMicrosoftServers = nil
		}
	}

	// Decode NiosMicrosoftAllocationJSON if the NIOS backup scan derived a
	// Microsoft allocation snapshot.
	var microsoftAllocation *MicrosoftAllocation
	if len(sess.NiosMicrosoftAllocationJSON) > 0 {
		microsoftAllocation = &MicrosoftAllocation{}
		if err := json.Unmarshal(sess.NiosMicrosoftAllocationJSON, microsoftAllocation); err != nil {
			fmt.Fprintf(os.Stderr, "server: failed to decode NiosMicrosoftAllocationJSON: %v\n", err)
			microsoftAllocation = nil
		}
	}
	...
	writeJSON(w, http.StatusOK, ScanResultsResponse{
		...
		NiosMicrosoftServers:  niosMicrosoftServers, // nil → omitted by omitempty
		MicrosoftAllocation:   microsoftAllocation,  // nil → omitted by omitempty
	})
}
