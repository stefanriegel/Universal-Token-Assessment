...
func TestOriginGuardRejectsCrossOriginMutation(t *testing.T) {
	...
	tests := []struct {
		...
		// Docker on a remote host, browsed by IP/hostname: Origin matches the
		// request's own Host -> same-origin -> allowed (regression, upload bug).
		{"same-origin remote host allowed", "http://192.168.6.14:8080", "192.168.6.14:8080", false},
		{"same-origin remote hostname allowed", "http://token-calc.corp:8080", "token-calc.corp:8080", false},
		// Cross-site attacker cannot forge a matching Origin for a remote host.
		{"cross-site against remote host rejected", "https://evil.example.com", "192.168.6.14:8080", true},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
