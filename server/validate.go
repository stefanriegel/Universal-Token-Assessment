...
func RegisterValidateHandler(r *chi.Mux, h *ValidateHandler) {
	...
	r.Post("/api/v1/providers/{provider}/device/start", h.HandleDeviceStart)
	r.Get("/api/v1/providers/{provider}/device/poll", h.HandleDevicePoll)
}
...
func (h *ValidateHandler) HandleValidate(w http.ResponseWriter, r *http.Request) {
	...
	h.establishSession(w, r, provider, req.AuthMethod, merged, req.ForestIndex)
...
// establishSession reuses the caller's session if one exists (multi-provider flow),
// otherwise creates a new one and sets the ddi_session cookie, then stores the
// validated credentials. Shared by HandleValidate and the two-phase device handlers.
// creds must be the merged map so tokens written back by validators (e.g.
// sso_access_token, azure_cred_cache_key) are captured.
func (h *ValidateHandler) establishSession(w http.ResponseWriter, r *http.Request, provider, authMethod string, creds map[string]string, forestIndex int) {
	...
	storeCredentials(sess, provider, authMethod, creds, forestIndex)
}
...
// onMessage, when non-nil, is called with the user-facing verification message
// (URL + code) as soon as it is known — before the blocking poll begins. This
// lets the two-phase device-auth handler surface the code to the browser
// immediately, which is the only way SSO works in a headless/Docker deployment
// where no browser can be launched on the server.
func realAWSSSO(ctx context.Context, creds map[string]string, onMessage func(string)) ([]SubscriptionItem, error) {
	...
	// Determine the verification URL and user code.
	openURL := ""
	...
	// Surface the code to the caller (two-phase device flow) so the browser can
	// display it. Works even when no server-side browser is available (Docker).
	if onMessage != nil {
		msg := fmt.Sprintf("To sign in, open %s in your browser", openURL)
		if userCode != "" {
			msg = fmt.Sprintf("To sign in, open %s and enter code %s", openURL, userCode)
		}
		onMessage(msg)
	}

	// Best-effort: also launch the system browser (desktop single-binary use).
	// The error is ignored — in a container there is no browser and the code
	// above already surfaced the URL to the user.
	if openURL != "" {
...
func realAWSValidator(ctx context.Context, creds map[string]string) ([]SubscriptionItem, error) {
	...
	case "sso":
		return realAWSSSO(ctx, creds, nil)
	case "org":
...
func realAzureBrowserSSO(ctx context.Context, creds map[string]string) ([]SubscriptionItem, error) {
	...
	// Browser SSO launches a browser on the machine running this binary. In a
	// container there is no browser (xdg-open/x-www-browser absent), so the SDK
	// fails with an exec lookup error. Device Code flow is the headless-friendly
	// alternative — the SDK returns a code + URL the user opens in their own browser.
	if isDocker() {
		return nil, errors.New("browser SSO is not available in Docker (no browser in the container) — use the \"Device Code\" authentication method instead")
	}
...
//
// onMessage, when non-nil, is called with the device-code message as soon as the
// SDK produces it (before the blocking poll), so the two-phase device-auth handler
// can surface the code to the browser immediately — the only way this works in a
// headless/Docker deployment where the code cannot be read from a terminal.
func realAzureDeviceCode(ctx context.Context, creds map[string]string, onMessage func(string)) ([]SubscriptionItem, error) {
	tenantID := creds["tenantId"]
	if tenantID == "" {
...
func realAzureValidator(ctx context.Context, creds map[string]string) ([]SubscriptionItem, error) {
	...
	case "device_code", "device-code":
		return realAzureDeviceCode(ctx, creds, nil)
...
// realGCPDeviceAuth implements GCP interactive auth via the OAuth2 device
// authorization flow (RFC 8628). Unlike realGCPBrowserOAuth's localhost-redirect
// flow — which needs the browser and the binary on the same machine and cannot
// work in Docker (the loopback redirect never reaches the container) — the device
// flow returns a verification URL + code the user enters on google.com/device in
// their OWN browser. No server-side browser, no redirect reachability required.
//
// onMessage, when non-nil, receives the verification message as soon as it is
// known, so the two-phase device-auth handler can surface it to the browser
// before the blocking token poll.
func realGCPDeviceAuth(ctx context.Context, creds map[string]string, onMessage func(string)) ([]SubscriptionItem, error) {
	clientID := creds["clientId"]
	clientSecret := creds["clientSecret"]
	if clientID == "" || clientSecret == "" {
		return nil, errors.New("clientId and clientSecret are required for browser OAuth — create OAuth (Desktop app) credentials in the GCP Console")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("gcp device auth: failed to start device authorization: %w", err)
	}

	// Surface the code + URL to the caller (two-phase flow) so the user can
	// complete consent in their own browser — works headless / in Docker.
	if onMessage != nil {
		msg := fmt.Sprintf("To sign in, open %s and enter code %s", da.VerificationURI, da.UserCode)
		onMessage(msg)
	}
	// Best-effort: open the pre-filled consent page on desktop single-binary use.
	...
	// Blocks until the user completes consent or the code expires.
	token, err := conf.DeviceAccessToken(ctx, da)
	if err != nil {
		return nil, fmt.Errorf("gcp device auth: token exchange failed: %w", err)
	}
	...
	items, err := listGCPAccessibleProjects(ctx, ts, "gcp device auth")
	if err != nil {
		return nil, err
	}
	if len(items) == 0 {
		return nil, errors.New("gcp device auth: no accessible GCP projects found for this account")
	}
	return items, nil
}
